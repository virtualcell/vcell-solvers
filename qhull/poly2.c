...
below %2.2g of the nearest %sfacet.\n",
  ...
  FOREACHfacet_i_(facets) {  /* for each point with facet assignment */
	...
	waserror= True;
	my_fprintf(qh ferr, "qhull precision error: point p%d is outside facet f%d, distance= %6.8g maxoutside= %6.8g\n", 
...
options 'Qci Tv' to verify all points.\n", notverified); 
  if (maxdist > qh outside_err) {
    my_fprintf( qh ferr, "qhull precision error (qh_check_bestdist): a coplanar point is %6.2g from convex hull.  The maximum value (qh.outside_err) is %6.2g\n",
...
void qh_check_maxout (void) {
  ...
  && (qh PRINTsummary || qh KEEPinside || qh KEEPcoplanar 
	...
	|| neighbor == qh tracefacet || vertex == qh tracevertex)
	  my_fprintf (qh ferr, "qh_check_maxout: p%d (v%d) is %.2g from f%d\n",
  ...
  do {
	...
	if (dist > qh TRACEdist || (bestfacet && bestfacet == qh tracefacet))
	  my_fprintf (qh ferr, "qh_check_maxout: p%d is %.2g above f%d\n",
...
all %sfacets.  Will make %2.0f distance computations.\n", 
    ...
    FORALLfacets {
      ...
      if (facet->flipped)
        continue;
      if (!facet->normal) {
	my_fprintf( qh ferr, "qhull warning (qh_check_points): missing normal for facet f%d\n", facet->id);
        continue;
      }
      if (testouter) {
#if qh_MAXoutside
    ...
    if (maxdist > qh outside_err) {
      my_fprintf( qh ferr, "qhull precision error (qh_check_points): a coplanar point is %6.2g from convex hull.  The maximum value (qh.outside_err) is %6.2g\n",
                maxdist, qh outside_err );
      qh_errexit2( qh_ERRprec, errfacet1, errfacet2 );
...
void qh_checkconvex(facetT *facetlist, int fault) {
  ...
  FORALLfacet_(facetlist) {
    if (facet->flipped) {
      ...
      my_fprintf (qh ferr, "qhull precision error: f%d is flipped (interior point is outside)\n",
	       facet->id);
      errfacet1= facet;
      waserror= True;
      continue;
	...
	if (!neighbor->simplicial || neighbor->tricoplanar) {
	  ...
	  if (fault == qh_DATAfault) {
	    ...
	    my_fprintf (qh ferr, "qhull precision error: initial simplex is not convex. Distance=%.2g\n", dist);
	    qh_errexit(qh_ERRsingular, NULL, NULL);
	  }
          if (dist > qh DISTround) {
            zzinc_(Zconcaveridges);
            qh_precision ("concave ridge");
            my_fprintf (qh ferr, "qhull precision error: f%d is concave to f%d, since p%d (v%d) is %6.4g above\n",
            ...
            if (dist > 0) {     /* qh_checkzero checks that dist < - qh DISTround */
              ...
              my_fprintf (qh ferr, "qhull precision error: f%d is clearly not convex to f%d, since p%d (v%d) is %6.4g above\n",
                facet->id, neighbor->id, qh_pointid(vertex->point), vertex->id, dist);
              errfacet1= facet;
              errfacet2= neighbor;
              waserror= True;
	...
	if (!centrum_warning && (!facet->simplicial || facet->tricoplanar)) {
	   ...
	   my_fprintf (qh ferr, "qhull note: recomputing centrums for convexity test.  This may lead to false, precision errors.\n");
	}
	...
	if (facet->tricoplanar || neighbor->tricoplanar)
	  continue;
        ...
        if (dist > qh DISTround) {
          zzinc_(Zconcaveridges);
          qh_precision ("concave ridge");
          my_fprintf (qh ferr, "qhull precision error: f%d is concave to f%d.  Centrum of f%d is %6.4g above f%d\n",
          ...
          zzinc_(Zcoplanarridges);
          qh_precision ("coplanar ridge");
          my_fprintf (qh ferr, "qhull precision error: f%d is coplanar or concave to f%d.  Centrum of f%d is %6.4g above f%d\n",
            facet->id, neighbor->id, facet->id, dist, neighbor->id);
	  errfacet1= facet;
	  errfacet2= neighbor;
	  waserror= True;
...
void qh_checkfacet(facetT *facet, boolT newmerge, boolT *waserrorp) {
  ...
  if (facet->visible) {
    my_fprintf (qh ferr, "qhull internal error (qh_checkfacet): facet f%d is on the visible_list\n",
      facet->id);
    qh_errexit (qh_ERRqhull, facet, NULL);
  }
  if (!facet->normal) {
    my_fprintf (qh ferr, "qhull internal error (qh_checkfacet): facet f%d does not have  a normal\n",
      facet->id);
    waserror= True;
  }
  ...
  FOREACHvertex_(facet->vertices) {
    if (vertex->deleted) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): deleted vertex v%d in f%d\n", vertex->id, facet->id);
      qh_errprint ("ERRONEOUS", NULL, NULL, NULL, vertex);
      waserror= True;
    }
    if (vertex->id >= previousid) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): vertices of f%d are not in descending id order at v%d\n", facet->id, vertex->id);
      waserror= True;
      break;
    }
  ...
  if (facet->simplicial) {
    ...
    && !facet->degenerate && !facet->redundant) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): for simplicial facet f%d, #vertices %d + #neighbors %d != 2*qh hull_dim\n", 
    ...
    && !facet->degenerate && !facet->redundant) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): for facet f%d, #vertices %d or #neighbors %d < qh hull_dim\n",
         facet->id, numvertices, numneighbors);
       waserror= True;
    }
	...
	my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): for facet f%d, #ridges %d < #neighbors %d or (3-d) > #vertices %d or (2-d) not all 2\n",
  ...
  FOREACHneighbor_(facet) {
    if (neighbor == qh_MERGEridge || neighbor == qh_DUPLICATEridge) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): facet f%d still has a MERGE or DUP neighbor\n", facet->id);
      qh_errexit (qh_ERRqhull, facet, NULL);
    }
    neighbor->seen= True;
  }
  FOREACHneighbor_(facet) {
    if (!qh_setin(neighbor->neighbors, facet)) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): facet f%d has neighbor f%d, but f%d does not have neighbor f%d\n",
	      facet->id, neighbor->id, neighbor->id, facet->id);
      errother= neighbor;
      waserror= True;
    }
    if (!neighbor->seen) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): facet f%d has a duplicate neighbor f%d\n",
  ...
  FOREACHridge_(facet->ridges) {
    if (ridge->seen) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): facet f%d has a duplicate ridge r%d\n",
	      facet->id, ridge->id);
      errridge= ridge;
      waserror= True;
    }    
    ...
    if (numRvertices != qh hull_dim - 1) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): ridge between f%d and f%d has %d vertices\n", 
    ...
    if (!qh_setin(facet->neighbors, neighbor)) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): for facet f%d, neighbor f%d of ridge r%d not in facet\n",
  ...
  if (!facet->simplicial) {
    FOREACHneighbor_(facet) {
      if (!neighbor->seen) {
        my_fprintf(qh ferr, "qhull internal error (qh_checkfacet): facet f%d does not have a ridge for neighbor f%d\n",
	      facet->id, neighbor->id);
	errother= neighbor;
        waserror= True;
      }
	...
	FOREACHvertex_(ridge->vertices) {
	  if (!vertex->seen) {
	    my_fprintf (qh ferr, "qhull internal error (qh_checkfacet): vertex v%d in r%d not in f%d intersect f%d\n",
	...
	FOREACHvertex_(intersection) {
	  if (!vertex->seen2) {
	    if (qh IStracing >=3 || !qh MERGING) {
	      my_fprintf (qh ferr, "qhull precision error (qh_checkfacet): vertex v%d in f%d intersect f%d but\n\
 not in a ridge.  This is ok under merging.  Last point was p%d\n",
	...
	if (!qh_setequal_skip (facet->vertices, skipA, neighbor->vertices, skipB)) {
	  my_fprintf (qh ferr, "qhull internal error (qh_checkfacet): facet f%d skip %d and neighbor f%d skip %d do not match \n",
  ...
  if (qh hull_dim < 5 && (qh IStracing > 2 || qh CHECKfrequently)) {
	...
	if (qh_setequal (ridge->vertices, ridge2->vertices)) {
	  my_fprintf (qh ferr, "qh_checkfacet: ridges r%d and r%d have the same vertices\n",
		  ridge->id, ridge2->id);
	  errridge= ridge;
	  waserror= True;
	}
...
void qh_checkflipped_all (facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    if (facet->normal && !qh_checkflipped (facet, &dist, !qh_ALL)) {
      my_fprintf(qh ferr, "qhull precision error: facet f%d is flipped, distance= %6.12g\n",
	      facet->id, dist);
      if (!qh FORCEoutput) {
...
void qh_checkpolygon(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
	...
	if (facet == qh facet_next)
	  ...
	  if (!qh NARROWhull
	    ...
	    my_fprintf (qh ferr, "qhull internal error (qh_checkpolygon): f%d has outside points before qh facet_next\n",
		     facet->id);
	    qh_errexit (qh_ERRqhull, facet, NULL);
	  }
	}
  ...
  if (qh visible_list && !visibleseen && facetlist == qh facet_list) {
    my_fprintf (qh ferr, "qhull internal error (qh_checkpolygon): visible list f%d no longer on facet list\n", qh visible_list->id);
    qh_printlists();
  ...
  FORALLfacet_(facetlist) {
	...
	if (qh_pointid (vertex->point) == -1) {
	  my_fprintf (qh ferr, "qhull internal error (qh_checkpolygon): unknown point %p for vertex v%d first_point %p\n",
  ...
  if (facetlist == qh facet_list) {
    if (numfacets != qh num_facets - qh num_visible) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkpolygon): actual number of facets is %d, cumulative facet count is %d - %d visible facets\n",
	...
	my_fprintf(qh ferr, "qhull internal error (qh_checkpolygon): vertex neighbors inconsistent.  Totvneighbors %d, totvertices %d\n",
		totvneighbors, totvertices);
	waserror= True;
      }
    }
    if (numvertices != qh num_vertices - qh_setsize(qh del_vertices)) {
      my_fprintf(qh ferr, "qhull internal error (qh_checkpolygon): actual number of vertices is %d, cumulative vertex count is %d\n",
	      numvertices, qh num_vertices - qh_setsize(qh del_vertices));
      waserror= True;
    }
    if (qh hull_dim == 2 && numvertices != numfacets) {
      my_fprintf (qh ferr, "qhull internal error (qh_checkpolygon): #vertices %d != #facets %d\n",
        numvertices, numfacets);
      waserror= True;
    }
    if (qh hull_dim == 3 && numvertices + numfacets - numridges/2 != 2) {
      my_fprintf (qh ferr, "qhull warning: #vertices %d + #facets %d - #edges %d != 2\n\
...
void qh_checkvertex (vertexT *vertex) {
  ...
  if (qh_pointid (vertex->point) == -1) {
    my_fprintf (qh ferr, "qhull internal error (qh_checkvertex): unknown point id %p\n", vertex->point);
    waserror= True;
  }
  if (vertex->id >= qh vertex_id) {
    my_fprintf (qh ferr, "qhull internal error (qh_checkvertex): unknown vertex id %d\n", vertex->id);
    waserror= True;
  }
    ...
    if (qh_setsize (vertex->neighbors)) {
      FOREACHneighbor_(vertex) {
        if (!qh_setin (neighbor->vertices, vertex)) {
          my_fprintf (qh ferr, "qhull internal error (qh_checkvertex): neighbor f%d does not contain v%d\n", neighbor->id, vertex->id);
	  errfacet= neighbor;
	  waserror= True;
	}
      }
...
setT *qh_facet3vertex (facetT *facet) {
  ...
  if (facet->simplicial) {
    if (cntvertices != 3) {
      my_fprintf (qh ferr, "qhull internal error (qh_facet3vertex): only %d vertices for simplicial facet f%d\n", 
    ...
    if (!ridge || cntprojected != cntvertices) {
      my_fprintf (qh ferr, "qhull internal error (qh_facet3vertex): ridges for facet %d don't match up.  got at least %d\n", 
  ...
  if (qh GOODvertex <0 || (qh GOODvertex > 0 && qh MERGING)) {
    FORALLfacet_(facetlist) {
      if (facet->good && ((qh GOODvertex > 0) ^ !!qh_isvertex (qh GOODvertexp, facet->vertices))) {
        if (!--numgood) {
	  if (qh ONLYgood) {
            my_fprintf (qh ferr, "qhull warning: good vertex p%d does not match last good facet f%d.  Ignored.\n",
               qh_pointid(qh GOODvertexp), facet->id);
	    return;
	  }else if (qh GOODvertex > 0)
            my_fprintf (qh ferr, "qhull warning: point p%d is not a vertex ('QV%d').\n",
		qh GOODvertex-1, qh GOODvertex-1);
	  else
            my_fprintf (qh ferr, "qhull warning: point p%d is a vertex for every facet ('QV-%d').\n",
	        -qh GOODvertex - 1, -qh GOODvertex - 1);
        }
        facet->good= False;
      }
...
void qh_infiniteloop (facetT *facet) {
  ...
  my_fprintf (qh ferr, "qhull internal error (qh_infiniteloop): potential infinite loop detected\n");
  qh_errexit (qh_ERRqhull, facet, NULL);
} /* qh_infiniteloop */
...
void qh_initbuild( void) {
  ...
  if ((qh GOODpoint  
    ...
    my_fprintf (qh ferr, "qhull input error: either QGn or QVn point is > p%d\n",
	     qh num_points-1);
  ...
  if (qh PRINToptions1st || qh TRACElevel || qh IStracing) {
    ...
    my_fprintf (qh ferr, "Options selected for Qhull %s:\n%s\n", qh_version, qh qhull_options);
  }
  ...
  if (qh ONLYgood) {
    if (qh GOODvertex > 0 && qh MERGING) {
      my_fprintf (qh ferr, "qhull input error: 'Qg QVn' (only good vertex) does not work with merging.\nUse 'QJ' to joggle the input or 'Q0' to turn off merging.\n");
      qh_errexit (qh_ERRinput, NULL, NULL);
    }
    if (!(qh GOODthreshold || qh GOODpoint
         || (!qh MERGEexact && !qh PREmerge && qh GOODvertexp))) {
      my_fprintf (qh ferr, "qhull input error: 'Qg' (ONLYgood) needs a good threshold ('Pd0D0'), a\n\
good point (QGn or QG-n), or a good vertex with 'QJ' or 'Q0' (QVn).\n");
	...
	&& !qh_isvertex (qh GOODvertexp, vertices)) {
      ...
      if (!isoutside) {
        my_fprintf (qh ferr, "qhull input error: point for QV%d is inside initial simplex.  It can not be made a vertex.\n",
...
void qh_initialhull(setT *vertices) {
  ...
  FORALLfacets {
    if (!qh_checkflipped (facet, NULL, !qh_ALL)) {  /* can happen with 'R0.1' */
      ...
      my_fprintf (qh ferr, "qhull precision error: initial facet %d is coplanar with the interior point\n",
                   facet->id);
  ...
  if (minangle < qh_MAXnarrow && !qh NOnarrow) { 
    ...
    if (minangle < qh_WARNnarrow && !qh RERUN && qh PRINTprecision)
      my_fprintf (qh ferr, "qhull precision warning: \n\
...
#ifndef qh_NOtrace
  if (qh IStracing >= 1) {
    my_fprintf(qh ferr, "qh_initialhull: simplex constructed, interior point:");
    for (k=0; k < qh hull_dim; k++) 
      my_fprintf (qh ferr, " %6.4g", qh interior_point[k]);
    my_fprintf (qh ferr, "\n");
  }
#endif
} /* initialhull */
  ...
  for (makematch= 0; makematch < 2; makematch++) {
	...
	if (qh_matchvertices (1, newfacet->vertices, newskip, facet->vertices, &skip, &same)) {
	  ...
	  if (SETelemt_(facet->neighbors, skip, facetT) != qh_DUPLICATEridge) {
	    if (!makematch) {
	      my_fprintf (qh ferr, "qhull internal error (qh_matchduplicates): missing dupridge at f%d skip %d for new f%d skip %d hash %d\n",
	...
	if (makematch && !facet 
	  ...
	  my_fprintf (qh ferr, "qhull internal error (qh_matchduplicates): no MERGEridge match for duplicate f%d skip %d at hash %d\n",
    ...
    if (!makematch) {
      if (!maxmatch) {
	my_fprintf (qh ferr, "qhull internal error (qh_matchduplicates): no maximum match at duplicate f%d skip %d at hash %d\n",
...
vertexT *qh_nearvertex (facetT *facet, pointT *point, realT *bestdistp) {
  ...
  if (facet->tricoplanar) {
    if (!qh VERTEXneighbors || !facet->center) {
      my_fprintf(qh ferr, "qhull internal error (qh_nearvertex): qh.VERTEXneighbors and facet->center required for tricoplanar facets\n");
      qh_errexit(qh_ERRqhull, facet, NULL);
    }
...
vertexT *qh_newvertex(pointT *point) {
  ...
  if (qh vertex_id == 0xFFFFFF) {
    my_fprintf(qh ferr, "qhull input error: more than %d vertices.  ID field overflows and two vertices\n\
...
void qh_point_add (setT *set, pointT *point, void *elem) {
  ...
  if ((id= qh_pointid(point)) < 0)
    my_fprintf (qh ferr, "qhull internal warning (point_add): unknown point %p id %d\n", 
      point, id);
  else if (id >= size) {
    my_fprintf (qh ferr, "qhull internal errror (point_add): point p%d is out of bounds (%d)\n",
	     id, size);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }else
...
*/
void qh_printhashtable(ostream* fp) {
  ...
  FOREACHfacet_i_(qh hash_table) {
    if (facet) {
      ...
      if (neighbor_i == neighbor_n)
        continue;
      my_fprintf (fp, "hash %d f%d ", facet_i, facet->id);
      FOREACHvertex_(facet->vertices)
        my_fprintf (fp, "v%d ", vertex->id);
      my_fprintf (fp, "\n neighbors:");
      FOREACHneighbor_i_(facet) {
...
void qh_printlists (void) {
  ...
  my_fprintf (qh ferr, "qh_printlists: facets:");
  FORALLfacets {
    if (++count % 100 == 0)
      my_fprintf (qh ferr, "\n     ");
    my_fprintf (qh ferr, " %d", facet->id);
  }
  ...
  FORALLvertices {
    if (++count % 100 == 0)
      my_fprintf (qh ferr, "\n     ");
    my_fprintf (qh ferr, " %d", vertex->id);
  }
  my_fprintf (qh ferr, "\n");
} /* printlists */
...
void qh_triangulate (void /*qh facet_list*/) {
  ...
  for (facet= new_facet_list; facet && facet->next; facet= nextfacet) { /* null facets moved to end */
	...
	my_fprintf( qh ferr, "qhull error (qh_triangulate): ridges still defined for f%d\n", facet->id);
	qh_errexit (qh_ERRqhull, facet, NULL);
      }
  ...
  for (facet= new_facet_list; facet && facet->next; facet= nextfacet) { /* may delete facet */
	...
	my_fprintf( qh ferr, "qhull error (qh_triangulate): tricoplanar facet f%d not owned by its visible, non-simplicial facet f%d\n", facet->id, getid_(visible));
...
void qh_triangulate_link (facetT *oldfacetA, facetT *facetA, facetT *oldfacetB, facetT *facetB) {
  ...
  if (qh_setin (facetA->neighbors, facetB)) {
    ...
    errmirror= True;
  if (errmirror) {
    my_fprintf( qh ferr, "qhull error (qh_triangulate_link): mirror facets f%d and f%d do not match for old facets f%d and f%d\n",
