...
void qh_initstatistics (void) {
	...
	if (!(qh_qhstat= (qhstatT *)malloc (sizeof(qhstatT)))) {
		my_fprintf_throw (qhmem.ferr, "qhull error (qh_initstatistics): insufficient memory\n");
	...
	if (qhstat next > sizeof(qhstat id)) {
		my_fprintf_throw (qhmem.ferr, "qhull error (qh_initstatistics): increase size of qhstat.id[].\n\
		...
		for (i=0; i < ZEND; i++) {
			...
			for (j=i+1; j < ZEND; j++) {
				if (qhstat id[i] == qhstat id[j]) {
					my_fprintf (qhmem.ferr, "qhull error (qh_initstatistics): duplicated statistic %d at indices %d and %d\n", 
					qhstat id[i], i, j);
				}
			}
		}
...
*/
void qh_printallstatistics (ostream* fp, char *string) {
...
*/
void qh_printstatistics (ostream* fp, char *string) {
	int i, k;
	realT ave;
	...
	if (qh premerge_cos < REALmax/2) my_fprintf (fp, "\
			%6.2g max. cosine for pre-merge angle\n", qh premerge_cos);
	if (qh PREmerge) my_fprintf (fp, "\
			%6.2g radius of pre-merge centrum\n", qh premerge_centrum);
	if (qh postmerge_cos < REALmax/2) my_fprintf (fp, "\
			%6.2g max. cosine for post-merge angle\n", qh postmerge_cos);
	if (qh POSTmerge) my_fprintf (fp, "\
	...
	for (k=0; k < qh hull_dim; k++)
		my_fprintf (fp, "%6.2e ", qh NEARzero[k]);
	my_fprintf (fp, "\n\n");
...
*/
void qh_printstatlevel (ostream* fp, int id, int start) {
	#define NULLfield "       "
	...
	if (qhstat count[id] != -1 
		...
		my_fprintf (fp, "%7.2g", qhstat stats[id].r);
	else if (qhstat type[id] >= ZTYPEreal && qhstat count[id] != -1)
		my_fprintf (fp, "%7.2g", qhstat stats[id].r/ qhstat stats[(unsigned char)(qhstat count[id])].i);
	else if (qhstat type[id] < ZTYPEreal && qhstat count[id] == -1)
		my_fprintf (fp, "%7d", qhstat stats[id].i);
	else if (qhstat type[id] < ZTYPEreal && qhstat count[id] != -1)
		my_fprintf (fp, "%7.3g", (realT) qhstat stats[id].i / qhstat stats[(unsigned char)(qhstat count[id])].i);
	my_fprintf (fp, " %s\n", qhstat doc[id]);
} /* printstatlevel */
...
*/
void qh_printstats (ostream* fp, int index, int *nextindex) {
	int j, nexti;
