...
void qh_qhull (void) {
  ...
  if (qh_setsize ((setT*)qhmem.tempstack) != 0) {
    my_fprintf (qh ferr, "qhull internal error (qh_qhull): temporary sets not empty (%d)\n",
...
boolT qh_addpoint (pointT *furthest, facetT *facet, boolT checkdist) {
	...
	if (!facet) {
		my_fprintf (qh ferr, "qh_addpoint: NULL facet.  Need to call qh_findbestfacet first\n");
		qh_errexit (qh_ERRqhull, NULL, NULL);
	}
	if (checkdist) {
...
void qh_buildhull(void) {
	...
	FORALLfacets {
		if (facet->visible || facet->newfacet) {
			my_fprintf (qh ferr, "qhull internal error (qh_buildhull): visible or new facet f%d in facet list\n",
						facet->id);    
	...
	FORALLvertices {
		if (vertex->newlist) {
			my_fprintf (qh ferr, "qhull internal error (qh_buildhull): new vertex f%d in vertex list\n",
						vertex->id);
	...
	if (qh num_outside && !furthest) {
		my_fprintf (qh ferr, "qhull internal error (qh_buildhull): %d outside points were never processed.\n", qh num_outside);
		qh_errexit (qh_ERRqhull, NULL, NULL);
	}
...
void qh_buildtracing (pointT *furthest, facetT *facet) {
	...
	outside points.  Next is point p%d (v%d), %2.2g above f%d.\n",
		...
		cpu /= qh_SECticks;
		qh_distplane (furthest, facet, &dist);
		my_fprintf (qh ferr, "qh_addpoint: add p%d (v%d) to hull of %d facets (%2.2g above f%d) and %d outside at %4.4g CPU secs.  Previous was p%d.\n",
	...
	*goodvisible= *goodhorizon= 0;
	...
	FORALLvisible_facets {
		if (visible->tricoplanar && !qh TRInormals) {
			my_fprintf (qh ferr, "qh_findhorizon: does not work for tricoplanar facets.  Use option 'Q11'\n");
	...
	if (!numhorizon) {
		...
		my_fprintf(qh ferr, "qhull precision error (qh_findhorizon): empty horizon\n\
...
pointT *qh_nextfurthest (facetT **visible) {
	...
	while ((facet= qh facet_next) != qh facet_tail) {
		...
		if (qh RANDOMoutside) {
			...
			my_fprintf (qh ferr, "qhull internal error (qh_nextfurthest): num_outside %d is too low\nby at least %d, or a random real %g >= 1.0\n",
...
void qh_partitioncoplanar (pointT *point, facetT *facet, realT *dist) {
  ...
  if (bestdist > qh max_outside) {
    ...
    if (bestdist > qh TRACEdist) {
      my_fprintf (qh ferr, "qh_partitioncoplanar: ====== p%d from f%d increases max_outside to %2.2g of f%d last p%d\n",
  ...
  *numoutside= 0;
  FORALLvisible_facets {
    ...
    if (newfacet == qh facet_tail) {
      my_fprintf (qh ferr, "qhull precision error (qh_partitionvisible): all new facets deleted as\n        degenerate facets. Can not continue.\n");
      qh_errexit (qh_ERRprec, NULL, NULL);
    }
...
/*-<a                             href="qh-qhull.htm#TOC"
  ...
  design:
    ...
    print summary
*/
void qh_printsummary(ostream* fp) {
...
Voronoi diagram by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    my_fprintf(fp, "  Number of Voronoi regions%s: %d\n",
              qh ATinfinity ? " and at-infinity" : "", numvertices);
    if (numdel)
      my_fprintf(fp, "  Total number of deleted points due to merging: %d\n", numdel); 
    if (numcoplanars - numdel > 0) 
      my_fprintf(fp, "  Number of nearly incident points: %d\n", numcoplanars - numdel); 
    else if (size - numvertices - numdel > 0) 
      my_fprintf(fp, "  Total number of nearly incident points: %d\n", size - numvertices - numdel); 
    my_fprintf(fp, "  Number of%s Voronoi vertices: %d\n", 
              goodused ? " 'good'" : "", qh num_good);
    if (nonsimplicial) 
      my_fprintf(fp, "  Number of%s non-simplicial Voronoi vertices: %d\n", 
...
Delaunay triangulation by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    my_fprintf(fp, "  Number of input sites%s: %d\n", 
              qh ATinfinity ? " and at-infinity" : "", numvertices);
    if (numdel)
      my_fprintf(fp, "  Total number of deleted points due to merging: %d\n", numdel); 
    if (numcoplanars - numdel > 0) 
      ...
      my_fprintf(fp, "  Total number of nearly incident points: %d\n", size - numvertices - numdel); 
    my_fprintf(fp, "  Number of%s Delaunay regions: %d\n", 
              goodused ? " 'good'" : "", qh num_good);
    if (nonsimplicial) 
      my_fprintf(fp, "  Number of%s non-simplicial Delaunay regions: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }else if (qh HALFspace) {
    my_fprintf (fp, "\n\
Halfspace intersection by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    my_fprintf(fp, "  Number of halfspaces: %d\n", size);
    my_fprintf(fp, "  Number of non-redundant halfspaces: %d\n", numvertices);
    if (numcoplanars) {
      if (qh KEEPinside && qh KEEPcoplanar)
        ...
        s= "similar"; 
      my_fprintf(fp, "  Number of %s halfspaces: %d\n", s, numcoplanars);
    } 
    my_fprintf(fp, "  Number of intersection points: %d\n", qh num_facets - qh num_visible);
    if (goodused)
      my_fprintf(fp, "  Number of 'good' intersection points: %d\n", qh num_good);
    if (nonsimplicial) 
      my_fprintf(fp, "  Number of%s non-simplicial intersection points: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }else {
    my_fprintf (fp, "\n\
Convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    my_fprintf(fp, "  Number of vertices: %d\n", numvertices);
    if (numcoplanars) {
      if (qh KEEPinside && qh KEEPcoplanar)
        ...
        s= "coplanar"; 
      my_fprintf(fp, "  Number of %s points: %d\n", s, numcoplanars);
    } 
    my_fprintf(fp, "  Number of facets: %d\n", qh num_facets - qh num_visible);
    if (goodused)
      my_fprintf(fp, "  Number of 'good' facets: %d\n", qh num_good);
    if (nonsimplicial) 
      my_fprintf(fp, "  Number of%s non-simplicial facets: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }
  if (numtricoplanars)
      my_fprintf(fp, "  Number of triangulated facets: %d\n", numtricoplanars);
  my_fprintf(fp, "\nStatistics for: %s | %s", 
  ...
  if (qh ROTATErandom != INT_MIN)
    ...
    my_fprintf(fp, "\n\n");
  my_fprintf(fp, "  Number of points processed: %d\n", zzval_(Zprocessed));
  my_fprintf(fp, "  Number of hyperplanes created: %d\n", zzval_(Zsetplane));
  if (qh DELAUNAY)
    my_fprintf(fp, "  Number of facets in hull: %d\n", qh num_facets - qh num_visible);
  my_fprintf(fp, "  Number of distance tests for qhull: %d\n", zzval_(Zpartition)+
      zzval_(Zpartitionall)+zzval_(Znumvisibility)+zzval_(Zpartcoplanar));
#if 0  /* NOTE: must print before printstatistics() */
  {realT stddev, ave;
  my_fprintf(fp, "  average new facet balance: %2.2g\n",
	  wval_(Wnewbalance)/zval_(Zprocessed));
  stddev= qh_stddev (zval_(Zprocessed), wval_(Wnewbalance), 
                                 wval_(Wnewbalance2), &ave);
  my_fprintf(fp, "  new facet standard deviation: %2.2g\n", stddev);
  my_fprintf(fp, "  average partition balance: %2.2g\n",
	  wval_(Wpbalance)/zval_(Zpbalance));
  stddev= qh_stddev (zval_(Zpbalance), wval_(Wpbalance), 
                                 wval_(Wpbalance2), &ave);
  my_fprintf(fp, "  partition standard deviation: %2.2g\n", stddev);
  }
#endif
  if (nummerged) {
    my_fprintf(fp,"  Number of distance tests for merging: %d\n",zzval_(Zbestdist)+
          zzval_(Zcentrumtests)+zzval_(Zdistconvex)+zzval_(Zdistcheck)+
          zzval_(Zdistzero));
    my_fprintf(fp,"  Number of distance tests for checking: %d\n",zzval_(Zcheckpart));
    my_fprintf(fp,"  Number of merged facets: %d\n", nummerged);
  }
  if (!qh RANDOMoutside && qh QHULLfinished) {
    cpu= qh hulltime;
    cpu /= qh_SECticks;
    wval_(Wcpu)= cpu;
    my_fprintf (fp, "  CPU seconds to compute hull (after input): %2.4g\n", cpu);
  }
  if (qh RERUN) {
    if (!qh PREmerge && !qh MERGEexact)
      my_fprintf(fp, "  Percentage of runs with precision errors: %4.1f\n",
	   zzval_(Zretry)*100.0/qh build_cnt);  /* careful of order */
  }else if (qh JOGGLEmax < REALmax/2) {
    if (zzval_(Zretry))
      my_fprintf(fp, "  After %d retries, input joggled by: %2.2g\n",
         zzval_(Zretry), qh JOGGLEmax);
    else
      my_fprintf(fp, "  Input joggled by: %2.2g\n", qh JOGGLEmax);
  }
  if (qh totarea != 0.0) 
    my_fprintf(fp, "  %s facet area:   %2.8g\n", 
	    zzval_(Ztotmerge) ? "Approximate" : "Total", qh totarea);
  if (qh totvol != 0.0) 
    my_fprintf(fp, "  %s volume:       %2.8g\n", 
	    zzval_(Ztotmerge) ? "Approximate" : "Total", qh totvol);
  if (qh MERGING) {
    qh_outerinner (NULL, &outerplane, &innerplane);
    if (outerplane > 2 * qh DISTround) {
      my_fprintf(fp, "  Maximum distance of %spoint above facet: %2.2g", 
    ...
    if (innerplane < -2 * qh DISTround) {
      my_fprintf(fp, "  Maximum distance of %svertex below facet: %2.2g", 
