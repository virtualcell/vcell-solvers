...
void qh_produce_output(void) {
  ...
  if (qh_setsize ((setT*)qhmem.tempstack) != tempsize) {
    my_fprintf (qh ferr, "qhull internal error (qh_produce_output): temporary sets not empty (%d)\n",
...
pointT *qh_detvnorm (vertexT *vertex, vertexT *vertexA, setT *centers, realT *offsetp) {
  ...
  if (numcenters > dim) {
    ...
    my_fprintf(qh ferr, "qh_detvnorm: too few points (%d) to compute separating plane\n", numcenters);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
  i= 0;
...
setT *qh_detvridge3 (vertexT *atvertex, vertexT *vertex) {
  ...
  if (qh CHECKfrequently) {
    FOREACHneighbor_(vertex) {
      if (!neighbor->seen2) {
	my_fprintf ((ostream*)(&cerr), "qh_detvridge3: neigbors of vertex p%d are not connected at facet %d\n",
...
void qh_order_vertexneighbors(vertexT *vertex) {
  ...
  while (qh_setsize (vertex->neighbors)) {
    ...
    if (!neighbor) {
      my_fprintf (qh ferr, "qhull internal error (qh_order_vertexneighbors): no neighbor of v%d for f%d\n",
        vertex->id, facet->id);
...
void qh_printafacet(ostream* fp, int format, facetT *facet, boolT printall) {
  ...
  case qh_PRINTarea:
    if (facet->isarea) {
      my_fprintf (fp, qh_REAL_1, facet->f.area);
      my_fprintf (fp, "\n");
    }else
      my_fprintf (fp, "0\n");
    break;
  case qh_PRINTcoplanars:
    my_fprintf (fp, "%d", qh_setsize (facet->coplanarset));
    FOREACHpoint_(facet->coplanarset)
      my_fprintf (fp, " %d", qh_pointid (point));
    my_fprintf (fp, "\n");
    break;
  ...
  LABELprintnorm:
    if (!facet->normal) {
      my_fprintf (fp, "no normal for facet f%d\n", facet->id);
      break;
    }
    if (qh CDDoutput) {
      my_fprintf (fp, qh_REAL_1, -offset);
      for (k=0; k < qh hull_dim; k++) 
	my_fprintf (fp, qh_REAL_1, -facet->normal[k]);
    }else {
      for (k=0; k < qh hull_dim; k++) 
	my_fprintf (fp, qh_REAL_1, facet->normal[k]);
      my_fprintf (fp, qh_REAL_1, offset);
  ...
  case qh_PRINTneighbors:
    my_fprintf (fp, "%d", qh_setsize (facet->neighbors));
    FOREACHneighbor_(facet)
      my_fprintf (fp, " %d", 
  ...
  case qh_PRINTpointintersect:
    if (!qh feasible_point) {
      my_fprintf (fp, "qhull input error (qh_printafacet): option 'Fp' needs qh feasible_point\n");
  ...
  LABELprintinfinite:
    for (k= qh hull_dim; k--; )
      my_fprintf (fp, qh_REAL_1, qh_INFINITE);
    my_fprintf (fp, "\n");   
    break;
  ...
  case qh_PRINTvertices:
    my_fprintf (fp, "%d", qh_setsize (facet->vertices));
    FOREACHvertex_(facet->vertices)
      my_fprintf (fp, " %d", qh_pointid (vertex->point));
    my_fprintf (fp, "\n");
    break;
  }
} /* printafacet */
...
void qh_printbegin (ostream* fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  case qh_PRINTgeom: 
    ...
    if (qh hull_dim == 2 && (qh PRINTridges || qh DOintersections))
      my_fprintf (qh ferr, "qhull warning: output for ridges and intersections not implemented in 2-d\n");
    if (qh hull_dim == 4 && (qh PRINTinner || qh PRINTouter ||
			     (qh PRINTdim == 4 && qh PRINTcentrums)))
      my_fprintf (qh ferr, "qhull warning: output for outer/inner planes and centrums not implemented in 4-d\n");
    if (qh PRINTdim == 4 && (qh PRINTspheres))
      my_fprintf (qh ferr, "qhull warning: output for vertices not implemented in 4-d\n");
    if (qh PRINTdim == 4 && qh DOintersections && qh PRINTnoplanes)
      my_fprintf (qh ferr, "qhull warning: 'Gnh' generates no output in 4-d\n");
    if (qh PRINTdim == 2) {
      my_fprintf(fp, "{appearance {linewidth 3} LIST # %s | %s\n",
	      qh rbox_command, qh qhull_command);
    }else if (qh PRINTdim == 3) {
      my_fprintf(fp, "{appearance {+edge -evert linewidth 2} LIST # %s | %s\n",
	      qh rbox_command, qh qhull_command);
	...
	num--;
      if (qh PRINTdim == 4)
        my_fprintf (fp, "4VECT %d %d 1\n", num, num);
      else
	my_fprintf (fp, "VECT %d %d 1\n", num, num);
      for (i= num; i--; ) {
        if (i % 20 == 0)
          my_fprintf (fp, "\n");
	my_fprintf (fp, "1 ");
      }
      my_fprintf (fp, "# 1 point per line\n1 ");
      for (i= num-1; i--; ) {
        if (i % 20 == 0)
          my_fprintf (fp, "\n");
	my_fprintf (fp, "0 ");
      }
      my_fprintf (fp, "# 1 color for all\n");
      FORALLpoints {
	...
	if (qh PRINTdim == 4)
      ...
      my_fprintf (fp, "0 1 1 1  # color of points\n");
    }
    if (qh PRINTdim == 4  && !qh PRINTnoplanes)
      /* 4dview loads up multiple 4OFF objects slowly */
      my_fprintf(fp, "4OFF %d %d 1\n", 3*qh ridgeoutnum, qh ridgeoutnum);
  ...
  case qh_PRINTincidences:
    if (qh VORONOI && qh PRINTprecision)
      my_fprintf (qh ferr, "qhull warning: writing Delaunay.  Use 'p' or 'o' for Voronoi centers\n");
  ...
  case qh_PRINTmaple:
    if (qh hull_dim > 3)  /* qh_initbuffers also checks */
      goto LABELnoformat;
    if (qh VORONOI)
      my_fprintf (qh ferr, "qhull warning: output is the Delaunay triangulation\n");
    if (format == qh_PRINTmaple) {
      if (qh hull_dim == 2)
	my_fprintf(fp, "PLOT(CURVES(\n");
      else
	my_fprintf(fp, "PLOT3D(POLYGONS(\n");
    }else
      my_fprintf(fp, "{\n");
  ...
  default:
  LABELnoformat:
    my_fprintf (qh ferr, "qhull internal error (qh_printbegin): can not use this format for dimension %d\n",
         qh hull_dim);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
} /* printbegin */
  ...
  if (qh CENTERtype == qh_ASvoronoi) {
    ...
    if (!facet->normal || !facet->upperdelaunay || !qh ATinfinity) {
      if (!facet->center)
        facet->center= qh_facetcenter (facet->vertices);
      for (k=0; k < num; k++)
        my_fprintf (fp, qh_REAL_1, facet->center[k]);
    }else {
      for (k=0; k < num; k++)
        my_fprintf (fp, qh_REAL_1, qh_INFINITE);
    }
...
void qh_printcentrum (ostream* fp, facetT *facet, realT radius) {
  ...
  my_fprintf (fp, "{appearance {-normal -edge normscale 0} ");
  if (qh firstcentrum) {
    qh firstcentrum= False;
    my_fprintf (fp, "{INST geom { define centrum CQUAD  # f%d\n\
-0.3 -0.3 0.0001     0 0 1 1\n\
 0.3 -0.3 0.0001     0 0 1 1\n\
 0.3  0.3 0.0001     0 0 1 1\n\
-0.3  0.3 0.0001     0 0 1 1 } transform { \n", facet->id);
  }else
    my_fprintf (fp, "{INST geom { : centrum } transform { # f%d\n", facet->id);
  ...
  my_fprintf (fp, "%8.4g %8.4g %8.4g 0\n", xaxis[0], xaxis[1], xaxis[2]);
...
void qh_printend (ostream* fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (!qh printoutnum)
    my_fprintf (qh ferr, "qhull warning: no facets printed\n");
  switch (format) {
  case qh_PRINTgeom:
	...
	my_fprintf (qh ferr, "qhull internal error (qh_printend): number of ridges %d != number printed %d and at end %d\n", qh ridgeoutnum, qh printoutvar, num);
	qh_errexit (qh_ERRqhull, NULL, NULL);
      }
    }else
      my_fprintf(fp, "}\n");
...
*/
void qh_printend4geom (ostream* fp, facetT *facet, int *nump, boolT printall) {
  realT color[3];
  int i, num= *nump;
  facetT *neighbor, **neighborp;
  ridgeT *ridge, **ridgep;
  ...
  if (facet->simplicial) {
	...
	if (fp)
          my_fprintf (fp, "3 %d %d %d %8.4g %8.4g %8.4g 1 # f%d f%d\n",
    ...
    FOREACHridge_(facet->ridges) {
      ...
      if (neighbor->visitid != qh visit_id) {
	if (fp)
          my_fprintf (fp, "3 %d %d %d %8.4g %8.4g %8.4g 1 #r%d f%d f%d\n",
		 3*num, 3*num+1, 3*num+2, color[0], color[1], color[2],
...
void qh_printextremes_2d (ostream* fp, facetT *facetlist, setT *facets, int printall) {
  ...
  do {
    ...
    if (facet->visitid == qh visit_id) {
      my_fprintf(qh ferr, "qh_printextremes_2d: loop in facet list.  facet %d nextfacet %d\n",
...
void qh_printfacet2math(ostream* fp, facetT *facet, int format, int notfirst) {
  ...
  my_fprintf(fp, pointfmt, point0[0], point0[1], point1[0], point1[1]);
  qh_memfree (point1, qh normal_size);
  qh_memfree (point0, qh normal_size);
} /* printfacet2math */
...
void qh_printfacet3math (ostream* fp, facetT *facet, int format, int notfirst) {
  ...
  FOREACHpoint_(points) {
    if (firstpoint)
      ...
      my_fprintf(fp, ",\n");
    my_fprintf(fp, pointfmt, point[0], point[1], point[2]);
  }
  FOREACHpoint_(points)
    qh_memfree (point, qh normal_size);
  qh_settempfree(&points);
  qh_settempfree(&vertices);
  my_fprintf(fp, endfmt);
} /* printfacet3math */
...
void qh_printfacet3vertex(ostream* fp, facetT *facet, int format) {
  ...
  if (format == qh_PRINToff)
    my_fprintf (fp, "%d ", qh_setsize (vertices));
  FOREACHvertex_(vertices) 
    my_fprintf (fp, "%d ", qh_pointid(vertex->point));
  my_fprintf (fp, "\n");
  qh_settempfree(&vertices);
...
void qh_printfacet4geom_nonsimplicial(ostream* fp, facetT *facet, realT color[3]) {
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if (qh DOintersections)
      ...
      if (qh DROPdim >= 0) 
	my_fprintf(fp, "OFF 3 1 1 # f%d\n", facet->id);
      else {
	qh printoutvar++;
	my_fprintf (fp, "# r%d between f%d f%d\n", ridge->id, facet->id, neighbor->id);
      }
	...
	qh_memfree (point, qh normal_size);
      ...
      if (qh DROPdim >= 0)
        my_fprintf(fp, "3 0 1 2 %8.4g %8.4g %8.4g\n", color[0], color[1], color[2]);
    }
  }
...
void qh_printfacet4geom_simplicial(ostream* fp, facetT *facet, realT color[3]) {
  ...
  FOREACHneighbor_(facet) {
    ...
    if (qh DOintersections)
      ...
      if (qh DROPdim >= 0) 
	my_fprintf(fp, "OFF 3 1 1 # ridge between f%d f%d\n",
		facet->id, neighbor->id);
      else {
	qh printoutvar++;
	my_fprintf (fp, "# ridge between f%d f%d\n", facet->id, neighbor->id);
      }
      FOREACHvertex_(vertices) {
	for(k= 0; k < qh hull_dim; k++) {
	  if (k != qh DROPdim)
  	    my_fprintf(fp, "%8.4g ", vertex->point[k]);
  	}
	my_fprintf (fp, "\n");
      }
      if (qh DROPdim >= 0) 
...
void qh_printfacetheader(ostream* fp, facetT *facet) {
  ...
  if (facet == qh_MERGEridge) {
    my_fprintf (fp, " MERGEridge\n");
    return;
  }else if (facet == qh_DUPLICATEridge) {
    my_fprintf (fp, " DUPLICATEridge\n");
    return;
  }else if (!facet) {
    my_fprintf (fp, " NULLfacet\n");
    return;
  }
  ...
  if (facet->toporient) 
    ...
    my_fprintf(fp, " bottom");
  if (facet->simplicial)
    my_fprintf(fp, " simplicial");
  if (facet->tricoplanar)
    my_fprintf(fp, " tricoplanar");
  if (facet->upperdelaunay)
    my_fprintf(fp, " upperDelaunay");
  if (facet->visible)
    my_fprintf(fp, " visible");
  if (facet->newfacet)
    my_fprintf(fp, " new");
  if (facet->tested)
    my_fprintf(fp, " tested");
  if (!facet->good)
    my_fprintf(fp, " notG");
  if (facet->seen)
    my_fprintf(fp, " seen");
  if (facet->coplanar)
    my_fprintf(fp, " coplanar");
  if (facet->mergehorizon)
    my_fprintf(fp, " mergehorizon");
  if (facet->keepcentrum)
    my_fprintf(fp, " keepcentrum");
  if (facet->dupridge)
    my_fprintf(fp, " dupridge");
  if (facet->mergeridge && !facet->mergeridge2)
    my_fprintf(fp, " mergeridge1");
  if (facet->mergeridge2)
    my_fprintf(fp, " mergeridge2");
  if (facet->newmerge)
    my_fprintf(fp, " newmerge");
  if (facet->flipped) 
    my_fprintf(fp, " flipped");
  if (facet->notfurthest) 
    my_fprintf(fp, " notfurthest");
  if (facet->degenerate)
    my_fprintf(fp, " degenerate");
  if (facet->redundant)
    my_fprintf(fp, " redundant");
  my_fprintf(fp, "\n");
  if (facet->isarea)
    my_fprintf(fp, "    - area: %2.2g\n", facet->f.area);
  else if (qh NEWfacets && facet->visible && facet->f.replace)
    my_fprintf(fp, "    - replacement: f%d\n", facet->f.replace->id);
  else if (facet->newfacet) {
    if (facet->f.samecycle && facet->f.samecycle != facet)
      my_fprintf(fp, "    - shares same visible/horizon as f%d\n", facet->f.samecycle->id);
  }else if (facet->tricoplanar /* !isarea */) {
    if (facet->f.triowner)
      my_fprintf(fp, "    - owner of normal & centrum is facet f%d\n", facet->f.triowner->id);
  }else if (facet->f.newcycle)
    my_fprintf(fp, "    - was horizon to f%d\n", facet->f.newcycle->id);
  if (facet->nummerge)
    my_fprintf(fp, "    - merges: %d\n", facet->nummerge);
  qh_printpointid(fp, "    - normal: ", qh hull_dim, facet->normal, -1);
  my_fprintf(fp, "    - offset: %10.7g\n", facet->offset);
  if (qh CENTERtype == qh_ASvoronoi || facet->center)
    qh_printcenter (fp, qh_PRINTfacets, "    - center: ", facet);
#if qh_MAXoutside
  if (facet->maxoutside > qh DISTround)
    my_fprintf(fp, "    - maxoutside: %10.7g\n", facet->maxoutside);
#endif
  if (!SETempty_(facet->outsideset)) {
    furthest= (pointT*)qh_setlast(facet->outsideset);
    if (qh_setsize (facet->outsideset) < 6) {
      my_fprintf(fp, "    - outside set (furthest p%d):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->outsideset)
	qh_printpoint(fp, "     ", point);
    }else if (qh_setsize (facet->outsideset) < 21) {
      qh_printpoints(fp, "    - outside set:", facet->outsideset);
    }else {
      my_fprintf(fp, "    - outside set:  %d points.", qh_setsize(facet->outsideset));
      qh_printpoint(fp, "  Furthest", furthest);
    }
#if !qh_COMPUTEfurthest
    my_fprintf(fp, "    - furthest distance= %2.2g\n", facet->furthestdist);
#endif
  }
  if (!SETempty_(facet->coplanarset)) {
    furthest= (pointT*)qh_setlast(facet->coplanarset);
    if (qh_setsize (facet->coplanarset) < 6) {
      my_fprintf(fp, "    - coplanar set (furthest p%d):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->coplanarset)
	qh_printpoint(fp, "     ", point);
    }else if (qh_setsize (facet->coplanarset) < 21) {
      qh_printpoints(fp, "    - coplanar set:", facet->coplanarset);
    }else {
      my_fprintf(fp, "    - coplanar set:  %d points.", qh_setsize(facet->coplanarset));
      qh_printpoint(fp, "  Furthest", furthest);
    }
    zinc_(Zdistio);
  ...
  my_fprintf(fp, "    - neighboring facets: ");
  FOREACHneighbor_(facet) {
...
void qh_printfacetridges(ostream* fp, facetT *facet) {
  ...
  if (facet->visible && qh NEWfacets) {
    my_fprintf(fp, "    - ridges (ids may be garbage):");
    FOREACHridge_(facet->ridges)
      my_fprintf(fp, " r%d", ridge->id);
    my_fprintf(fp, "\n");
  }else {
    my_fprintf(fp, "    - ridges:\n");
    FOREACHridge_(facet->ridges)
      ridge->seen= False;
    if (qh hull_dim == 3) {
...
void qh_printfacets(ostream* fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (qh CDDoutput && (format == qh_PRINTcentrums || format == qh_PRINTpointintersect || format == qh_PRINToff))
    my_fprintf (qh ferr, "qhull warning: CDD format is not available for centrums, halfspace\nintersections, and OFF file format.\n");
  if (format == qh_PRINTnone)
    ...
    my_fprintf(fp, "Options selected for Qhull %s:\n%s\n", qh_version, qh qhull_options);
    ...
    my_fprintf (fp, "0\n2 ");
    my_fprintf (fp, qh_REAL_1, qh totarea);
    my_fprintf (fp, qh_REAL_1, qh totvol);
    my_fprintf (fp, "\n");
  }else if (format == qh_PRINTsummary) {
    qh_countfacets (facetlist, facets, printall, &numfacets, &numsimplicial, 
      &totneighbors, &numridges, &numcoplanars, &numtricoplanars);
    vertices= qh_facetvertices (facetlist, facets, printall); 
    my_fprintf (fp, "10 %d %d %d %d %d %d %d %d %d %d\n2 ", qh hull_dim, 
    ...
    my_fprintf (fp, qh_REAL_2n, outerplane, innerplane);
...
to the center point are as follows:\n\n", qh DISTround);
  ...
  FORALLfacets {
    my_fprintf (fp, "facet");
    FOREACHvertex_(facet->vertices)
      my_fprintf (fp, " p%d", qh_pointid(vertex->point));
    zinc_(Zdistio);
    qh_distplane(qh interior_point, facet, &dist);
    my_fprintf (fp, " distance= %4.2g\n", dist);
  }
  if (!qh_QUICKhelp) {
    if (qh HALFspace) 
      my_fprintf (fp, "\n\
...
the points by randomly rotating the input with 'QR0'.\n");
  ...
  for (k=0; k < qh hull_dim; k++) {
    ...
    my_fprintf (fp, "  %d:  %8.4g  %8.4g  difference= %4.4g\n", k, min, max, max-min);
  }
  if (!qh_QUICKhelp) {
    my_fprintf (fp, "\n\
...
void qh_printhyperplaneintersection(ostream* fp, facetT *facet1, facetT *facet2,
  ...
  if (qh hull_dim == 3)
    my_fprintf(fp, "VECT 1 %d 1 %d 1 ", i, i);
  else if (qh hull_dim == 4 && qh DROPdim >= 0)
    my_fprintf(fp, "OFF 3 1 1 ");
  else
    qh printoutvar++;
  my_fprintf (fp, "# intersect f%d f%d\n", facet1->id, facet2->id);
  ...
  FOREACHvertex_(vertices) {
    ...
    if (qh PRINTdim <= 3) {
      qh_projectdim3 (p, p);
      my_fprintf(fp, "%8.4g %8.4g %8.4g # ", p[0], p[1], p[2]);
    }else 
      my_fprintf(fp, "%8.4g %8.4g %8.4g %8.4g # ", p[0], p[1], p[2], p[3]);
    if (nearzero1+nearzero2)
      my_fprintf (fp, "p%d (coplanar facets)\n", qh_pointid (vertex->point));
    else
      my_fprintf (fp, "projected p%d\n", qh_pointid (vertex->point));
  }
  if (qh hull_dim == 3)
    my_fprintf(fp, "%8.4g %8.4g %8.4g 1.0\n", color[0], color[1], color[2]); 
  else if (qh hull_dim == 4 && qh DROPdim >= 0)  
    my_fprintf(fp, "3 0 1 2 %8.4g %8.4g %8.4g 1.0\n", color[0], color[1], color[2]);
...
void qh_printline3geom (ostream* fp, pointT *pointA, pointT *pointB, realT color[3]) {
  ...
  if ((fabs(pA[0] - pB[0]) > 1e-3) || 
      (fabs(pA[1] - pB[1]) > 1e-3) || 
      (fabs(pA[2] - pB[2]) > 1e-3)) {
    my_fprintf (fp, "VECT 1 2 1 2 1\n");
    for (k= 0; k < 3; k++)
       my_fprintf (fp, "%8.4g ", pB[k]);
    my_fprintf (fp, " # p%d\n", qh_pointid (pointB));
  }else
    my_fprintf (fp, "VECT 1 1 1 1 1\n");
  for (k=0; k < 3; k++)
    my_fprintf (fp, "%8.4g ", pA[k]);
  my_fprintf (fp, " # p%d\n", qh_pointid (pointA));
  my_fprintf (fp, "%8.4g %8.4g %8.4g 1\n", color[0], color[1], color[2]);
}
...
void qh_printpointid(ostream* fp, char *string, int dim, pointT *point, int id) {
  ...
  if (string) {
    my_fputs (string, fp);
   if (id != -1)
      my_fprintf(fp, " p%d: ", id);
  }
  for(k= dim; k--; ) {
    r= *point++;
    if (string)
      my_fprintf(fp, " %8.4g", r);
    else
      my_fprintf(fp, qh_REAL_1, r);
  }
  my_fprintf(fp, "\n");
} /* printpointid */
...
void qh_printridge(ostream* fp, ridgeT *ridge) {
  ...
  if (ridge->tested)
    my_fprintf (fp, " tested");
  if (ridge->nonconvex)
    my_fprintf (fp, " nonconvex");
  my_fprintf (fp, "\n");
  qh_printvertices (fp, "           vertices:", ridge->vertices);
  if (ridge->top && ridge->bottom)
    my_fprintf(fp, "           between f%d and f%d\n",
...
void qh_printspheres(ostream* fp, setT *vertices, realT radius) {
  ...
  qh printoutnum++;
  my_fprintf (fp, "{appearance {-edge -normal normscale 0} {\n\
...
void qh_printvdiagram (ostream* fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (format == qh_PRINTvertices) {
    ...
    my_fprintf(qh ferr, "qh_printvdiagram: unknown print format %d.\n", format);
    qh_errexit (qh_ERRinput, NULL, NULL);
  }
...
void qh_printvertex(ostream* fp, vertexT *vertex) {
  ...
  if (!vertex) {
    my_fprintf (fp, "  NULLvertex\n");
    return;
  }
  my_fprintf(fp, "- p%d (v%d):", qh_pointid(vertex->point), vertex->id);
  point= vertex->point;
  if (point) {
    for(k= qh hull_dim; k--; ) {
      r= *point++;
      my_fprintf(fp, " %5.2g", r);
    }
  }
  if (vertex->deleted)
    my_fprintf(fp, " deleted");
  if (vertex->delridge)
    my_fprintf (fp, " ridgedeleted");
  my_fprintf(fp, "\n");
  if (vertex->neighbors) {
    my_fprintf(fp, "  neighbors:");
    FOREACHneighbor_(vertex) {
      if (++count % 100 == 0)
	my_fprintf (fp, "\n     ");
      my_fprintf(fp, " f%d", neighbor->id);
...
void qh_printvoronoi (ostream* fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (format == qh_PRINTgeom) 
    my_fprintf (fp, "{appearance {+edge -face} OFF %d %d 1 # Voronoi centers and cells\n", 
                numcenters, numvertices);
  else
    my_fprintf (fp, "%d\n%d %d 1\n", qh hull_dim-1, numcenters, qh_setsize(vertices));
  if (format == qh_PRINTgeom) {
    for (k= qh hull_dim-1; k--; )
      my_fprintf (fp, qh_REAL_1, 0.0);
    my_fprintf (fp, " 0 # infinity not used\n");
  }else {
    for (k= qh hull_dim-1; k--; )
      my_fprintf (fp, qh_REAL_1, qh_INFINITE);
    my_fprintf (fp, "\n");
	...
	if (vertex) {
	  FOREACHneighbor_(vertex) {
	    if (neighbor->visitid && neighbor->visitid < numfacets)
	      my_fprintf (fp, " %d", neighbor->visitid);
	  }
	}
	my_fprintf (fp, " # p%d (v%d)\n", vertex_i, vertex->id);
      }else
	my_fprintf (fp, " # p%d is coplanar or isolated\n", vertex_i);
    }else {
      if (numinf)
	numneighbors++;
      my_fprintf (fp, "%d", numneighbors);
...
void qh_printvnorm (ostream* fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT unbounded) {
  ...
  for (k= 0; k< qh hull_dim-1; k++)
    my_fprintf (fp, qh_REAL_1, normal[k]);
  my_fprintf (fp, qh_REAL_1, offset);
  my_fprintf (fp, "\n");
} /* printvnorm */
...
void qh_printvridge (ostream* fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT unbounded) {
  ...
  my_fprintf (fp, "%d %d %d", qh_setsize (centers)+2, 
       qh_pointid (vertex->point), qh_pointid (vertexA->point));
  FOREACHfacet_(centers) 
    my_fprintf (fp, " %d", facet->visitid);
  my_fprintf (fp, "\n");
} /* printvridge */
...
int qh_readfeasible (int dim, char *remainder) {
  ...
  if (!qh HALFspace) {
    my_fprintf  (qh ferr, "qhull input error: feasible point (dim 1 coords) is only valid for halfspace intersection\n");
    qh_errexit (qh_ERRinput, NULL, NULL);
  }  
  if (qh feasible_string)
    my_fprintf  (qh ferr, "qhull input warning: feasible point (dim 1 coords) overrides 'Hn,n,n' feasible point for halfspace intersection\n");
  if (!(qh feasible_point= (coordT*)malloc (dim* sizeof(coordT)))) {
    my_fprintf(qh ferr, "qhull error: insufficient memory for feasible point\n");
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  coords= qh feasible_point;
  while ((s= (isfirst ?  remainder : my_fgets(firstline, qh_MAXfirst, qh fin)))) {
    if (isfirst)
      isfirst= False;
    else
      linecount++;
    while (*s) {
      ...
      *(coords++)= value;
      if (++tokcount == dim) {
        ...
        qh_strtod (s, &t);
        if (s != t) {
          my_fprintf (qh ferr, "qhull input error: coordinates for feasible point do not finish out the line: %s\n",
               s);
          qh_errexit (qh_ERRinput, NULL, NULL);
        }
        return linecount;
  ...
  my_fprintf (qh ferr, "qhull input error: only %d coordinates.  Could not read %d-d feasible point.\n",
           tokcount, dim);
  qh_errexit (qh_ERRinput, NULL, NULL);
  return 0;
} /* readfeasible */
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
	...
	if (qh CDDinput) {
		while ((s= my_fgets(firstline, qh_MAXfirst, qh fin))) {
		linecount++;
		...
		if (!s) {
		my_fprintf (qh ferr, "qhull input error: missing \"begin\" for cdd-formated input\n");
		qh_errexit (qh_ERRinput, NULL, NULL);
		}
	}
	while(!numinput && (s= my_fgets(firstline, qh_MAXfirst, qh fin))) {
		linecount++;
	...
	if (!s) {
		my_fprintf(qh ferr, "qhull input error: short input file.  Did not find dimension and number of points\n");
		qh_errexit(qh_ERRinput, NULL, NULL);
	}
	...
	if (diminput < 2) {
		my_fprintf(qh ferr,"qhull input error: dimension %d (first number) should be at least 2\n",
			diminput);
		qh_errexit(qh_ERRinput, NULL, NULL);
	}
	if (isdelaunay) {
		...
		*numpoints= numinput;
		if (diminput < 3) {
		my_fprintf(qh ferr,"qhull input error: dimension %d (first number, includes offset) should be at least 3 for halfspaces\n",
  			diminput);
		qh_errexit(qh_ERRinput, NULL, NULL);
		}
		if (dimfeasible) {
		if (dimfeasible != *dimension) {
			my_fprintf(qh ferr,"qhull input error: dimension %d of feasible point is not one less than dimension %d for halfspaces\n",
			dimfeasible, diminput);
			qh_errexit(qh_ERRinput, NULL, NULL);
		}
		}else 
	...
	*ismalloc= True;  /* use malloc since memory not setup */
	...
	if (!coords || !qh line || (qh HALFspace && !qh half_space)) {
		my_fprintf(qh ferr, "qhull error: insufficient memory to read %d points\n",
			numinput);
		qh_errexit(qh_ERRmem, NULL, NULL);
	}
	...
	paraboloid= 0.0;
	while ((s= (isfirst ?  s : my_fgets(qh line, qh maxline, qh fin)))) {
		if (!isfirst) {
		linecount++;
		if (*s == 'e' || *s == 'E') {
		...
		if (qh CDDinput )
			...
			my_fprintf (qh ferr, "qhull input warning: the input appears to be in cdd format.  If so, use 'Fd'\n");
		}
		}
		}
		islong= False;
		...
		*(coordp++)= value;
			...
			*(coords++)= value;
			...
			if (value != 1.0) {
				my_fprintf (qh ferr, "qhull input error: for cdd format, point at line %d does not start with '1'\n",
					linecount);
				qh_errexit (qh_ERRinput, NULL, NULL);
			}
			coords--;
		...
		if (++coordcount == diminput) {
			...
			*(coords++)= paraboloid;
			...
			my_fprintf (qh ferr, "The halfspace was on line %d\n", linecount);
			if (wasbegin)
			my_fprintf (qh ferr, "The input appears to be in cdd format.  If so, you should use option 'Fd'\n");
			qh_errexit (qh_ERRinput, NULL, NULL);
		}
		...
		my_fprintf(qh ferr, "qhull input error: line %d contained more than %d characters\n", 
	...
	%d points and %d extra coordinates.  Line %d is the first\npoint",
		...
		if (firsttext)
		my_fprintf(qh ferr, ", line %d is the first comment", firsttext);
		if (firstshort)
		my_fprintf(qh ferr, ", line %d is the first short\nline", firstshort);
		if (firstlong)
		my_fprintf(qh ferr, ", line %d is the first long line", firstlong);
		my_fprintf(qh ferr, ".  Continue with %d points.\n", newnum);
		numinput= newnum;
		if (isdelaunay && qh ATinfinity) {
...
Either prepend the input with 1 point or use 'Hn,n,n'.  See manual.\n");
  ...
  if (!(qh feasible_point= (pointT*)malloc (dim* sizeof(coordT)))) {
    my_fprintf(qh ferr, "qhull error: insufficient memory for 'Hn,n,n'\n");
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  coords= qh feasible_point;
  while (*s) {
    value= qh_strtod (s, &s);
    if (++tokcount > dim) {
      my_fprintf (qh ferr, "qhull input warning: more coordinates for 'H%s' than dimension %d\n",
