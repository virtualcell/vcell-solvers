...
#if 0
{
	...
	ostream* outfile= &cout; //stdout;    
					/* output from qh_produce_output()
					use NULL to skip qh_produce_output() */
	ostream* errfile= &cerr; //stderr;    /* error messages from qhull code */
	...
	if (curlong || totlong) 
		my_fprintf (errfile, "qhull internal warning (main): did not free %d bytes of long memory (%d pieces)\n", totlong, curlong);
}
#endif
...
int qh_new_qhull (int dim, int numpoints, coordT *points, boolT ismalloc, 
		char *qhull_cmd, ostream* outfile, ostream* errfile) {
  ...
  if (strncmp (qhull_cmd,"qhull ", 6)) {
    my_fprintf_throw (errfile, "qh_new_qhull: start qhull_cmd argument with \"qhull \"\n");
    //exit(1);
  }
...
void qh_errexit(int exitcode, facetT *facet, ridgeT *ridge) {
	...
	if (qh NOerrexit) {
		my_fprintf_throw (qh ferr, "qhull error while ending program.  Exit program\n");
		//exit(1);
	}
	qh NOerrexit= True;
...
void qh_errprint(char *string, facetT *atfacet, facetT *otherfacet, ridgeT *atridge, vertexT *atvertex) {
	...
	if (atfacet) {
		my_fprintf(qh ferr, "%s FACET:\n", string);
		qh_printfacet(qh ferr, atfacet);
	}
	if (otherfacet) {
		my_fprintf(qh ferr, "%s OTHER FACET:\n", string);
		qh_printfacet(qh ferr, otherfacet);
	}
	if (atridge) {
		my_fprintf(qh ferr, "%s RIDGE:\n", string);
	...
	if (atvertex) {
		my_fprintf(qh ferr, "%s VERTEX:\n", string);
		qh_printvertex (qh ferr, atvertex);
	}
	if (qh fout && qh FORCEoutput && atfacet && !qh QHULLfinished && !qh IStracing) {
		my_fprintf(qh ferr, "ERRONEOUS and NEIGHBORING FACETS to output\n");
