...
void qh_appendmergeset(facetT *facet, facetT *neighbor, mergeType mergetype, realT *angle) {
  ...
  if (mergetype < MRGdegen)
    ...
    if (facet->redundant || neighbor->redundant) {
      my_fprintf(qh ferr, "qhull error (qh_appendmergeset): facet f%d or f%d is already a mirrored facet\n",
	   facet->id, neighbor->id);
      qh_errexit2 (qh_ERRqhull, facet, neighbor);
    }
    if (!qh_setequal (facet->vertices, neighbor->vertices)) {
      my_fprintf(qh ferr, "qhull error (qh_appendmergeset): mirrored facets f%d and f%d do not have the same vertices\n",
	   facet->id, neighbor->id);
      qh_errexit2 (qh_ERRqhull, facet, neighbor);
    }
    facet->redundant= True;
...
void qh_checkconnect (void /* qh newfacet_list */) {
  ...
  FORALLnew_facets {
    if (newfacet->visitid == qh visit_id)
      break;
    my_fprintf(qh ferr, "qhull error: f%d is not attached to the new facets\n",
         newfacet->id);
    errfacet= newfacet;
  }
  if (errfacet)
...
#ifndef qh_NOtrace
  if (qh IStracing >= 4) {
    my_fprintf (qh ferr, "qh_find_newvertex: find new vertex for v%d from ",
	     oldvertex->id);
    FOREACHvertex_(vertices) 
      my_fprintf (qh ferr, "v%d ", vertex->id);
    FOREACHridge_(ridges)
      my_fprintf (qh ferr, "r%d ", ridge->id);
    my_fprintf (qh ferr, "\n");
  }
#endif
  FOREACHvertex_(vertices) 
...
facetT *qh_findbestneighbor(facetT *facet, realT *distp, realT *mindistp, realT *maxdistp) {
  ...
  *distp= REALmax;
  ...
  if (!bestfacet) {
    my_fprintf (qh ferr, "qhull internal error (qh_findbestneighbor): no neighbors for f%d\n", facet->id);
...
void qh_forcedmerges(boolT *wasmerge) {
  ...
  FOREACHmerge_(othermerges) {
    ...
    if (!qh_setin (facet2->neighbors, facet1)) {
      my_fprintf (qh ferr, "qhull internal error (qh_forcedmerges): f%d and f%d had a duplicate ridge but as f%d and f%d they are no longer neighbors\n",
...
int qh_merge_degenredundant (void) {
  ...
  while ((merge= (mergeT*)qh_setdellast (qh degen_mergeset))) {
    ...
    if (mergetype == MRGredundant) {
      ...
      while (facet2->visible) {
        if (!facet2->f.replace) {
          my_fprintf (qh ferr, "qhull internal error (qh_merge_degenredunant): f%d redundant but f%d has no replacement\n",
	       facet1->id, facet2->id);
          qh_errexit2 (qh_ERRqhull, facet1, facet2);
        }
...
void qh_mergecycle (facetT *samecycle, facetT *newfacet) {
  ...
  if (newfacet->tricoplanar) {
    if (!qh TRInormals) {
      my_fprintf (qh ferr, "qh_mergecycle: does not work for tricoplanar facets.  Use option 'Q11'\n");
  ...
  if (newfacet == qh tracefacet) {
    ...
    qh IStracing= 4;
    my_fprintf (qh ferr, "qh_mergecycle: ========= trace merge %d of samecycle %d into trace f%d, furthest is p%d\n",
	       zzval_(Ztotmerge), samecycle->id, newfacet->id,  qh furthest_id);
    traceonce= True;
  }
  if (qh IStracing >=4) {
    my_fprintf (qh ferr, "  same cycle:");
    FORALLsame_cycle_(samecycle)
      my_fprintf(qh ferr, " f%d", same->id);
    my_fprintf (qh ferr, "\n");
  }
  if (qh IStracing >=4)
  ...
  if (traceonce) {
    my_fprintf (qh ferr, "qh_mergecycle: end of trace facet\n");
...
void qh_mergecycle_all (facetT *facetlist, boolT *wasmerge) {
  ...
  for (facet= facetlist; facet && (nextfacet= facet->next); facet= nextfacet) {
    ...
    if (!facet->mergehorizon) {
      my_fprintf (qh ferr, "qh_mergecycle_all: f%d without normal\n", facet->id);
      qh_errexit (qh_ERRqhull, facet, NULL);
    }
    horizon= SETfirstt_(facet->neighbors, facetT);
...
void qh_mergecycle_ridges(facetT *samecycle, facetT *newfacet) {
  ...
  FORALLsame_cycle_(samecycle) {
	...
	my_fprintf (qh ferr, "qhull internal error (qh_mergecycle_ridges): bad ridge r%d\n", ridge->id);
...
void qh_mergefacet(facetT *facet1, facetT *facet2, realT *mindist, realT *maxdist, boolT mergeapex) {
  ...
  if (facet1->tricoplanar || facet2->tricoplanar) {
    if (!qh TRInormals) {
      my_fprintf (qh ferr, "qh_mergefacet: does not work for tricoplanar facets.  Use option 'Q11'\n");
      qh_errexit2 (qh_ERRqhull, facet1, facet2);
    }
  ...
  if (qh build_cnt >= qh RERUN) {
    if (mindist && (-*mindist > qh TRACEdist || *maxdist > qh TRACEdist)) {
      ...
      traceonce= True;
      my_fprintf (qh ferr, "qh_mergefacet: ========= trace wide merge #%d (%2.2g) for f%d into f%d, last point was p%d\n", zzval_(Ztotmerge),
	     fmax_(-*mindist, *maxdist), facet1->id, facet2->id, qh furthest_id);
    }else if (facet1 == qh tracefacet || facet2 == qh tracefacet) {
      tracerestore= qh IStracing;
      qh IStracing= 4;
      traceonce= True;
      my_fprintf (qh ferr, "qh_mergefacet: ========= trace merge #%d involving f%d, furthest is p%d\n",
  ...
  if (qh IStracing >= 2) {
    ...
    my_fprintf (qh ferr, "qh_mergefacet: #%d merge f%d into f%d, mindist= %2.2g, maxdist= %2.2g\n", 
    zzval_(Ztotmerge), facet1->id, facet2->id, mergemin, mergemax);
  }
#endif /* !qh_NOtrace */
  if (facet1 == facet2 || facet1->visible || facet2->visible) {
    my_fprintf (qh ferr, "qhull internal error (qh_mergefacet): either f%d and f%d are the same or one is a visible facet\n",
	     facet1->id, facet2->id);
    qh_errexit2 (qh_ERRqhull, facet1, facet2);
  }
...
too strong.\n", qh hull_dim+1);
    if (qh hull_dim >= 5 && !qh MERGEexact)
      my_fprintf(qh ferr, "Option 'Qx' may avoid this problem.\n");
  ...
  if (traceonce) {
    my_fprintf (qh ferr, "qh_mergefacet: end of wide tracing\n");
    qh IStracing= tracerestore;
  }
} /* mergefacet */
...
void qh_mergevertex_neighbors(facetT *facet1, facetT *facet2) {
  ...
  trace4((qh ferr, "qh_mergevertex_neighbors: merge vertex neighbors of f%d and f%d\n",
	  facet1->id, facet2->id));
  if (qh tracevertex) {
    my_fprintf (qh ferr, "qh_mergevertex_neighbors: of f%d and f%d at furthest p%d f0= %p\n",
...
void qh_mergevertices(setT *vertices1, setT **vertices2) {
  ...
  if (newsize < qh_setsize (mergedvertices)) {
    my_fprintf (qh ferr, "qhull internal error (qh_mergevertices): facets did not share a ridge\n");
...
vertexT *qh_rename_sharedvertex (vertexT *vertex, facetT *facet) {
  ...
  if (qh_setsize (vertex->neighbors) == 2) {
    ...
    if (!neighborA) {
      my_fprintf (qh ferr, "qhull internal error (qh_rename_sharedvertex): v%d's neighbors not in f%d\n",
        vertex->id, facet->id);
...
void qh_renamevertex(vertexT *oldvertex, vertexT *newvertex, setT *ridges, facetT *oldfacet, facetT *neighborA) {
  ...
  if (!oldfacet) {
    zinc_(Zrenameall);
    if (istrace)
      my_fprintf (qh ferr, "qh_renamevertex: renamed v%d to v%d in several facets\n",
    ...
    zinc_(Zrenameshare);
    if (istrace)
      my_fprintf (qh ferr, "qh_renamevertex: renamed v%d to v%d in oldfacet f%d\n", 
    ...
    if (istrace || qh IStracing)
      my_fprintf (qh ferr, "qh_renamevertex: renamed pinched v%d to v%d between f%d and f%d\n", 
...
void qh_tracemerge (facetT *facet1, facetT *facet2) {
  ...
  if (facet2 == qh tracefacet || (qh tracevertex && qh tracevertex->newlist)) {
    my_fprintf (qh ferr, "qh_tracemerge: trace facet and vertex after merge of f%d and f%d, furthest p%d\n", facet1->id, facet2->id, qh furthest_id);
  ...
  if (qh tracevertex) {
    if (qh tracevertex->deleted)
      my_fprintf (qh ferr, "qh_tracemerge: trace vertex deleted at furthest p%d\n",
	    qh furthest_id);
    else
