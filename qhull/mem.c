...
void *qh_memalloc(int insize) {
  ...
  if ((unsigned) insize <= (unsigned) qhmem.LASTsize) {
	...
	if (!(newbuffer= malloc(bufsize))) {
	  my_fprintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
	...
	*((void **)newbuffer)= qhmem.curbuffer;  /* prepend newbuffer to curbuffer 
    ...
    if (!qhmem.indextable) {
      my_fprintf (qhmem.ferr, "qhull internal error (qh_memalloc): qhmem has not been initialized.\n");
    ...
    if (qhmem.IStracing >= 5)
      my_fprintf (qhmem.ferr, "qh_memalloc long: %d bytes at %p\n", outsize, object);
  }
  return (object);
} /* memalloc */
...
void qh_memfree(void *object, int size) {
  ...
  if (size <= qhmem.LASTsize) {
    ...
    *freelistp= object;
    ...
    free (object);
    if (qhmem.IStracing >= 5)
      my_fprintf (qhmem.ferr, "qh_memfree long: %d bytes at %p\n", size, object);
  }
} /* memfree */
...
*/
void qh_meminit (ostream* ferr) {
  
  memset((char *)&qhmem, 0, sizeof qhmem);  /* every field is 0, FALSE, NULL */
  qhmem.ferr= ferr;
  if (sizeof(void*) < sizeof(int)) {
    my_fprintf_throw (ferr, "qhull internal error (qh_meminit): sizeof(void*) < sizeof(int).  qset.c will not work\n");
...
void qh_meminitbuffers (int tracelevel, int alignment, int numsizes, int bufsize, int bufinit) {
  ...
  if (qhmem.ALIGNmask & ~qhmem.ALIGNmask) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_meminit): memory alignment %d is not a power of 2\n", alignment);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  ...
  if (!qhmem.sizetable || !qhmem.freelists) {
    my_fprintf(qhmem.ferr, "qhull error (qh_meminit): insufficient memory\n");
    qh_errexit (qhmem_ERRmem, NULL, NULL);
  }
  if (qhmem.IStracing >= 1)
    my_fprintf (qhmem.ferr, "qh_meminitbuffers: memory initialized with alignment %d\n", alignment);
} /* meminitbuffers */
...
void qh_memsetup (void) {
  ...
  if (qhmem .LASTsize >= qhmem .BUFsize || qhmem.LASTsize >= qhmem .BUFinit) {
    my_fprintf (qhmem.ferr, "qhull error (qh_memsetup): largest mem size %d is >= buffer size %d or initial buffer size %d\n",
            qhmem .LASTsize, qhmem .BUFsize, qhmem .BUFinit);
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
  if (!(qhmem.indextable= (int *)malloc((qhmem.LASTsize+1) * sizeof(int)))) {
    my_fprintf(qhmem.ferr, "qhull error (qh_memsetup): insufficient memory\n");
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
...
void qh_memsize(int size) {
  ...
  if (qhmem .LASTsize) {
    my_fprintf (qhmem .ferr, "qhull error (qh_memsize): called after qhmem_setup\n");
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  ...
  if (qhmem.TABLEsize < qhmem.NUMsizes)
    ...
    my_fprintf(qhmem.ferr, "qhull warning (memsize): free list table has room for only %d sizes\n", qhmem.NUMsizes);
} /* memsize */
...
*/
void qh_memstatistics (ostream* fp) {
...
%7d bytes per memory buffer (initially %d bytes)\n",
  ...
  if (qhmem.cntlarger) {
    my_fprintf (fp, "%7d calls to qh_setlarger\n%7.2g     average copy size\n",
	   qhmem.cntlarger, ((float) qhmem.totlarger)/ qhmem.cntlarger);
    my_fprintf (fp, "  freelists (bytes->count):");
  }
  for (i=0; i < qhmem.TABLEsize; i++) {
    count=0;
    for (object= qhmem .freelists[i]; object; object= *((void **)object))
      count++;
    my_fprintf (fp, " %d->%d", qhmem.sizetable[i], count);
  }
  my_fprintf (fp, "\n\n");
} /* memstatistics */
...
void qh_meminit (ostream* ferr) {
  ...
  qhmem.ferr= ferr;
  if (sizeof(void*) < sizeof(int)) {
    my_fprintf (ferr, "qhull internal error (qh_meminit): sizeof(void*) < sizeof(int).  qset.c will not work\n");
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
}
