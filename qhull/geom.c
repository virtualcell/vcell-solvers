...
void qh_distplane (pointT *point, facetT *facet, realT *dist) {
  ...
  if (qh IStracing >= 4) {
    my_fprintf (qh ferr, "qh_distplane: ");
    my_fprintf (qh ferr, qh_REAL_1, *dist);
    my_fprintf (qh ferr, "from p%d to f%d\n", qh_pointid(point), facet->id);
  }
  return;
} /* distplane */
...
facetT *qh_findbest (pointT *point, facetT *startfacet, 
  ...
  if (qh IStracing >= 3 || (qh TRACElevel && qh TRACEpoint >= 0 && qh TRACEpoint == qh_pointid (point))) {
    ...
    my_fprintf (qh ferr, "qh_findbest: point p%d starting at f%d isnewfacets? %d, unless %d exit if > %2.2g\n",
	     qh_pointid(point), startfacet->id, isnewfacets, bestoutside, qh MINoutside);
    my_fprintf(qh ferr, "  testhorizon? %d noupper? %d", testhorizon, noupper);
    my_fprintf (qh ferr, "  Last point added was p%d.", qh furthest_id);
    my_fprintf(qh ferr, "  Last merge was #%d.  max_outside %2.2g\n", zzval_(Ztotmerge), qh max_outside);
  }
  if (isoutside)
    *isoutside= True;
...
facetT *qh_findbestnew (pointT *point, facetT *startfacet,
  ...
  if (!startfacet) {
    if (qh MERGING)
      my_fprintf(qh ferr, "qhull precision error (qh_findbestnew): merging has formed and deleted a cone of new facets.  Can not continue.\n");
    else
      my_fprintf(qh ferr, "qhull internal error (qh_findbestnew): no new facets for point p%d\n",
      	      qh furthest_id);      
  ...
  *numpart= 0;
  if (qh IStracing >= 3 || (qh TRACElevel && qh TRACEpoint >= 0 && qh TRACEpoint == qh_pointid (point))) {
    if (qh TRACElevel > qh IStracing)
      qh IStracing= qh TRACElevel;
    my_fprintf(qh ferr, "qh_findbestnew: point p%d facet f%d. Stop? %d if dist > %2.2g\n",
	     qh_pointid(point), startfacet->id, isdistoutside, distoutside);
    my_fprintf(qh ferr, "  Last point added p%d visitid %d.",  qh furthest_id, visitid);
    my_fprintf(qh ferr, "  Last merge was #%d.\n", zzval_(Ztotmerge));
  }
...
void qh_gausselim(realT **rows, int numrow, int numcol, boolT *sign, boolT *nearzero) {
  ...
  *nearzero= False;
  for(k= 0; k < numrow; k++) {
	...
	if (qh IStracing >= 4) {
	  my_fprintf (qh ferr, "qh_gausselim: 0 pivot at column %d. (%2.2g < %2.2g)\n", k, pivot_abs, qh DISTround);
...
pointT *qh_getcenter(setT *vertices) {
  ...
  if (count < 2) {
    my_fprintf (qh ferr, "qhull internal error (qh_getcenter): not defined for %d points\n", count);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
...
void qh_setfacetplane(facetT *facet) {
  ...
  if (facet == qh tracefacet) {
    ...
    qh IStracing= 5;
    my_fprintf (qh ferr, "qh_setfacetplane: facet f%d created.\n", facet->id);
    my_fprintf (qh ferr, "  Last point added to hull was p%d.", qh furthest_id);
    if (zzval_(Ztotmerge))
      my_fprintf(qh ferr, "  Last merge was #%d.", zzval_(Ztotmerge));
    my_fprintf (qh ferr, "\n\nCurrent summary is:\n");
  ...
  if (qh PRINTstatistics || qh IStracing || qh TRACElevel || qh JOGGLEmax < REALmax) {
	...
	zinc_(Zdiststat);
	  ...
	  istrace= True;
	if (istrace) {
	  my_fprintf (qh ferr, "qh_setfacetplane: ====== vertex p%d (v%d) increases max_outside to %2.2g for new facet f%d last p%d\n",
  ...
  if (qh IStracing >= 3) {
    my_fprintf (qh ferr, "qh_setfacetplane: f%d offset %2.2g normal: ",
	     facet->id, facet->offset);
    for (k=0; k < qh hull_dim; k++)
      my_fprintf (qh ferr, "%2.2g ", facet->normal[k]);
    my_fprintf (qh ferr, "\n");
  }
  if (facet == qh tracefacet)
    qh IStracing= oldtrace;
