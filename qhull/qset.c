...
void qh_setaddnth(setT **setp, int nth, void *newelem) {
  ...
  if (nth < 0 || nth > oldsize) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_setaddnth): nth %d is out-of-bounds for set:\n", nth);
...
void qh_setcheck(setT *set, char *tname, int id) {
  ...
  if (size > maxsize || !maxsize) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_setcheck): actual size %d of %s%d is greater than max size %d\n",
	     size, tname, id, maxsize);
    waserr= 1;
  }else if (set->e[size].p) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_setcheck): %s%d (size %d max %d) is not null terminated.\n",
...
/*-<a                             href="qh-set.htm#TOC"
  ...
  notes:
    never errors
*/
void qh_setprint(ostream* fp, char* string, setT *set) {
  int size, k;

  if (!set)
    my_fprintf (fp, "%s set is null\n", string);
  else {
    SETreturnsize_(set, size);
    my_fprintf (fp, "%s set=%p maxsize=%d size=%d elems=",
	     string, set, set->maxsize, size);
    if (size > set->maxsize)
      size= set->maxsize+1;
    for (k=0; k < size; k++)
      my_fprintf(fp, " %p", set->e[k].p);
    my_fprintf(fp, "\n");
  }
} /* setprint */
...
void qh_setreplace(setT *set, void *oldelem, void *newelem) {
  ...
  if (*elemp)
    *elemp= newelem;
    ...
    my_fprintf (qhmem.ferr, "qhull internal error (qh_setreplace): elem %p not found in set\n",
       oldelem);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
...
int qh_setsize(setT *set) {
  ...
  if ((size= *sizep)) {
    size--;
    if (size > set->maxsize) {
      my_fprintf (qhmem.ferr, "qhull internal error (qh_setsize): current set size %d is greater than maximum size %d\n",
	       size, set->maxsize);
...
setT *qh_settemp(int setsize) {
  ...
  if (qhmem.IStracing >= 5)
    my_fprintf (qhmem.ferr, "qh_settemp: temp set %p of %d elements, depth %d\n",
...
void qh_settempfree(setT **set) {
  ...
  if (stackedset != *set) {
    ...
    my_fprintf (qhmem.ferr, "qhull internal error (qh_settempfree): set %p (size %d) was not last temporary allocated (depth %d, set %p, size %d)\n",
...
setT *qh_settemppop(void) {
  ...
  if (!stackedset) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_settemppop): pop from empty temporary stack\n");
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  if (qhmem.IStracing >= 5)
    my_fprintf (qhmem.ferr, "qh_settemppop: depth %d temp set %p of %d elements\n",
...
void qh_settemppush(setT *set) {
  ...
  if (qhmem.IStracing >= 5)
    my_fprintf (qhmem.ferr, "qh_settemppush: depth %d temp set %p of %d elements\n",
...
void qh_settruncate (setT *set, int size) {
  ...
  if (size < 0 || size > set->maxsize) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_settruncate): size %d out of bounds for set:\n", size);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
...
void qh_setzero (setT *set, int index, int size) {
  ...
  if (index < 0 || index >= size || size > set->maxsize) {
    my_fprintf (qhmem.ferr, "qhull internal error (qh_setzero): index %d or size %d out of bounds for set:\n", index, size);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
