...
coordT *qh_copypoints (coordT *points, int numpoints, int dimension) {
  ...
  if (!(newpoints=(coordT*)malloc(size))) {
    my_fprintf(qh ferr, "qhull error: insufficient memory to copy %d points\n",
        numpoints);
...
realT qh_determinant (realT **rows, int dim, boolT *nearzero) {
  ...
  *nearzero= False;
  if (dim < 2) {
    my_fprintf (qh ferr, "qhull internal error (qh_determinate): only implemented for dimension >= 2\n");
...
void qh_detroundoff (void) {
  ...
  { /* compute ONEmerge, max vertex offset for merging simplicial facets */
    ...
    qh ONEmerge= sqrt ((double)qh hull_dim) * qh MAXwidth *
      sqrt ((double)(1.0 - maxangle * maxangle)) + qh DISTround;  
  ...
  if (qh JOGGLEmax < qh DISTround) {
    my_fprintf (qh ferr, "qhull error: the joggle for 'QJn', %.2g, is below roundoff for distance computations, %.2g\n",
  ...
  && !qh BESToutside && !qh FORCEoutput)
    my_fprintf (qh ferr, "qhull input warning: minimum visibility V%.2g is greater than \nminimum outside W%.2g.  Flipped facets are likely.\n",
	     qh MINvisible, qh MINoutside);
...
realT qh_detsimplex(pointT *apex, setT *points, int dim, boolT *nearzero) {
  ...
  if (i < dim) {
    my_fprintf (qh ferr, "qhull internal error (qh_detsimplex): #points %d < dimension %d\n", 
               i, dim);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
...
realT qh_facetarea_simplex (int dim, coordT *apex, setT *vertices, 
  ...
  if (i != dim-1) {
    my_fprintf (qh ferr, "qhull internal error (qh_facetarea_simplex): #points %d != dim %d -1\n", 
               i, dim);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
  rows[i]= gmcoord;
...
void qh_getarea (facetT *facetlist) {
  ...
  if (qh REPORTfreq)
    my_fprintf (qh ferr, "computing area of each facet and volume of the convex hull\n");
  else 
...
void qh_joggleinput (void) {
  ...
  if (!qh input_points) { /* first call */
    ...
    if (!(qh first_point=(coordT*)malloc(size))) {
      my_fprintf(qh ferr, "qhull error: insufficient memory to joggle %d points\n",
          qh num_points);
      qh_errexit(qh_ERRmem, NULL, NULL);
    }
  ...
  if (qh build_cnt > 1 && qh JOGGLEmax > fmax_(qh MAXwidth/4, 0.1)) {
      my_fprintf (qh ferr, "qhull error: the current joggle for 'QJn', %.2g, is too large for the width\nof the input.  If possible, recompile Qhull with higher-precision reals.\n",
	        qh JOGGLEmax);
      qh_errexit (qh_ERRqhull, NULL, NULL);
  }
...
setT *qh_maxmin(pointT *points, int numpoints, int dimension) {
  ...
  && REALmax > 0.0 && -REALmax < 0.0)
    ...
    my_fprintf (qh ferr, "qhull error: floating point constants in user.h are wrong\n\
...
void qh_maxsimplex (int dim, setT *maxpoints, pointT *points, int numpoints, setT **simplex) {
  ...
  if (sizinit < 2) {
	...
	my_fprintf (qh ferr, "qhull precision error (qh_maxsimplex for voronoi_center):\n%d points with the same x coordinate.\n",
		 qh_setsize(maxpoints)+numpoints);
	qh_errexit (qh_ERRprec, NULL, NULL);
      }else {
	my_fprintf (qh ferr, "qhull input error: input is less than %d-dimensional since it has the same x coordinate\n", qh hull_dim);
	qh_errexit (qh_ERRinput, NULL, NULL);
      }
    }
  }
  for(k= sizinit; k < dim+1; k++) {
    ...
    if (!maxpoint) {
      my_fprintf (qh ferr, "qhull internal error (qh_maxsimplex): not enough points available\n");
      qh_errexit (qh_ERRqhull, NULL, NULL);
    }
...
*/
void qh_printmatrix (ostream* fp, char *string, realT **rows, int numrow, int numcol) {
  realT *rowp;
  realT r; /*bug fix*/
  int i,k;
...
*/
void qh_printpoints (ostream* fp, char *string, setT *points) {
  pointT *point, **pointp;

  if (string) {
    my_fprintf (fp, "%s", string);
    FOREACHpoint_(points) 
      my_fprintf (fp, " p%d", qh_pointid(point));
    my_fprintf (fp, "\n");
  }else {
    FOREACHpoint_(points) 
...
void qh_projectinput (void) {
  ...
  if (newdim != qh hull_dim) {
    my_fprintf(qh ferr, "qhull internal error (qh_projectinput): dimension after projection %d != hull_dim %d\n", newdim, qh hull_dim);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
  if (!(newpoints=(coordT*)malloc(newnum*newdim*sizeof(coordT)))){
    my_fprintf(qh ferr, "qhull error: insufficient memory to project %d points\n",
           qh num_points);
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  ...
  if (qh HALFspace) {
    if (!qh feasible_point) {
      my_fprintf(qh ferr, "qhull internal error (qh_projectinput): HALFspace defined without qh.feasible_point\n");
      qh_errexit(qh_ERRqhull, NULL, NULL);
    }
...
void qh_projectpoints (signed char *project, int n, realT *points, 
  ...
  for (k= 0; k < n; k++)
    testdim += project[k];
  if (testdim != newdim) {
    my_fprintf (qh ferr, "qhull internal error (qh_projectpoints): newdim %d should be %d after projection\n",
      newdim, testdim);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
  for (j= 0; j<n; j++) {
...
void qh_scalelast (coordT *points, int numpoints, int dim, coordT low,
  ...
  if (nearzero) {
    if (qh DELAUNAY)
      my_fprintf (qh ferr, "qhull input error: can not scale last coordinate.  Input is cocircular\n   or cospherical.   Use option 'Qz' to add a point at infinity.\n");
    else
      my_fprintf (qh ferr, "qhull input error: can not scale last coordinate.  New bounds [0, %2.2g] are too wide for\nexisting bounds [%2.2g, %2.2g] (width %2.2g)\n",
...
void qh_scalepoints (pointT *points, int numpoints, int dim,
  ...
  for (k= 0; k < dim; k++) {
    ...
    if (qh DELAUNAY && k == dim-1 && newhigh < newlow) {
      my_fprintf (qh ferr, "qhull input error: 'Qb%d' or 'QB%d' inverts paraboloid since high bound %.2g < low bound %.2g\n",
	       k, k, newhigh, newlow);
      qh_errexit (qh_ERRinput, NULL, NULL);
    }
    scale= qh_divzero (newhigh - newlow, high - low,
                  qh MINdenom_1, &nearzero);
    if (nearzero) {
      my_fprintf (qh ferr, "qhull input error: %d'th dimension's new bounds [%2.2g, %2.2g] too wide for\nexisting bounds [%2.2g, %2.2g]\n",
...
boolT qh_sethalfspace (int dim, coordT *coords, coordT **nextp, 
  ...
  *nextp= coordp;
  if (qh IStracing >= 4) {
    my_fprintf (qh ferr, "qh_sethalfspace: halfspace at offset %6.2g to point: ", *offset);
    for (k= dim, coordp= coords; k--; ) {
      r= *coordp++;
      my_fprintf (qh ferr, " %6.2g", r);
    }
    my_fprintf (qh ferr, "\n");
  }
  return True;
LABELerroroutside:
  feasiblep= feasible;
  normp= normal;
  my_fprintf(qh ferr, "qhull input error: feasible point is not clearly inside halfspace\nfeasible point: ");
  for (k= dim; k--; )
    my_fprintf (qh ferr, qh_REAL_1, r=*(feasiblep++));
  my_fprintf (qh ferr, "\n     halfspace: "); 
  for (k= dim; k--; )
    my_fprintf (qh ferr, qh_REAL_1, r=*(normp++));
  my_fprintf (qh ferr, "\n     at offset: ");
  my_fprintf (qh ferr, qh_REAL_1, *offset);
  my_fprintf (qh ferr, " and distance: ");
  my_fprintf (qh ferr, qh_REAL_1, dist);
  my_fprintf (qh ferr, "\n");
  return False;
} /* sethalfspace */
...
coordT *qh_sethalfspace_all (int dim, int count, coordT *halfspaces, pointT *feasible) {
  ...
  if (!(newpoints=(coordT*)malloc(count*newdim*sizeof(coordT)))){
    my_fprintf(qh ferr, "qhull error: insufficient memory to compute dual of %d halfspaces\n",
          count);
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  coordp= newpoints;
  ...
  for (i= 0; i < count; i++) {
    ...
    if (!qh_sethalfspace (newdim, coordp, &coordp, normalp, offsetp, feasible)) {
      my_fprintf (qh ferr, "The halfspace was at index %d\n", i);
      qh_errexit (qh_ERRinput, NULL, NULL);
    }
    normalp= offsetp + 1;
  }
...
pointT *qh_voronoi_center (int dim, setT *points) {
  ...
  if (size == dim+1)
    ...
    my_fprintf (qh ferr, "qhull internal error (qh_voronoi_center):\n  need at least %d points to construct a Voronoi center\n",
	     dim+1);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }else {
...
#ifndef qh_NOtrace
    if (qh IStracing >= 3) {
      my_fprintf (qh ferr, "qh_voronoi_center: det %2.2g factor %2.2g ", det, factor);
      qh_printmatrix (qh ferr, "center:", &center, 1, dim);
      if (qh IStracing >= 5) {
	qh_printpoints (qh ferr, "points", simplex);
	FOREACHpoint_(simplex)
	  my_fprintf (qh ferr, "p%d dist %.2g, ", qh_pointid (point),
