...
void qh_attachnewfacets (void ) {
  ...
  FORALLnew_facets {
	...
	my_fprintf (qh ferr, "qhull internal error (qh_attachnewfacets): couldn't find visible facet for horizon f%d of newfacet f%d\n",
...
void qh_deletevisible (void /*qh visible_list*/) {
  ...
  if (numvisible != qh num_visible) {
    my_fprintf (qh ferr, "qhull internal error (qh_deletevisible): qh num_visible %d is not number of visible facets %d\n",
...
setT *qh_facetintersect (facetT *facetA, facetT *facetB,
  ...
  if (i >= dim || j >= dim) {
    my_fprintf (qh ferr, "qhull internal error (qh_facetintersect): f%d or f%d not in others neighbors\n",
            facetA->id, facetB->id);
	...
	newfacet->mergehorizon= True;
      ...
      if (qh ONLYgood) {
 	  ...
 	  qh_setappend(&(newfacet->ridges), ridge);
        ...
        if (neighbor->seen) {
	  if (neighbor->simplicial) {
	    my_fprintf (qh ferr, "qhull internal error (qh_makenew_nonsimplicial): simplicial f%d sharing two ridges with f%d\n", 
  ...
  for (scan= hash; (facet= SETelemt_(qh hash_table, scan, facetT)); 
    ...
    if (qh_matchvertices (1, newfacet->vertices, newskip, facet->vertices, &skip, &same)) {
      if (SETelem_(newfacet->vertices, newskip) == 
        ...
        my_fprintf (qh ferr, "qhull precision error: Vertex sets are the same for f%d and f%d.  Can not force output.\n",
	...
	my_fprintf (qh ferr, "qhull precision error: facets f%d, f%d and f%d meet at a ridge with more than 2 neighbors.  Can not continue.\n",
...
void qh_matchnewfacets (void /* qh newfacet_list */) {
  ...
  FORALLnew_facets {
	...
	if (facet == newfacet)
      ...
      if (count != hashcount) {
	my_fprintf (qh ferr, "qh_matchnewfacets: after adding facet %d, hashcount %d != count %d\n",
  ...
  if (hashcount) {
    my_fprintf (qh ferr, "qhull internal error (qh_matchnewfacets): %d neighbors did not match up\n",
        hashcount);
  ...
  if (qh IStracing >= 2) {
    ...
    my_fprintf (qh ferr, "qh_matchnewfacets: %d new facets, %d unused hash entries .  hashsize %d\n",
