...
input.  Permitted input strings are ÒsolutionÓ, ÒfrontÓ, ÒbackÓ, ÒupÓ, ÒdownÓ,
ÒfsolnÓ, ÒbsolnÓ, and ÒallÓ.  Returns MSnone if input is ÒnoneÓ or is not
recognized. */
...
input.  Permitted input strings are ÒsystemÓ and ÒportÓ.  Returns MLTnone for all
other input. */
...
msptr is returned pointing to whatever state is requested; ÒnoneÓ is not a
...
the molecule name is unknown, and -5 implies that the molecule name is ÒallÓ. */
...
list (i.e. not a box list) in m; if itÕs unknown set m to -1.  If it is to be
bound to a panel, or if it was on a panel, enter the panel in pnl (both are not
possible since a molecule canÕt hop from one panel to another with this
...
Either or both ÒallÓ conditions are permitted.  This does not update rms step
...
ÒallÓ conditions are permitted.  This returns 0 for successful operation and 1
...
MSall, this sets the drift vector for all states.  Either or both ÒallÓ
...
ms is MSall, this sets the display size for all states.  Either or both ÒallÓ
...
both ÒallÓ conditions are permitted. */
...
Either or both of i and ms can be set to ÒallÓ; for i enter a negative number
...
element set to -1).  Because this shouldnÕt normally be called with ll>0 and
...
molecule in the master list (i.e. not a box list) in m; if itÕs unknown set m
...
and/or ps to a negative number, or pname to ÒallÓ, to indicate ÒallÓ; in this
...
success, 2 if cmpt->npts is 0, or 3 if there arenÕt enough available
molecules. */
...
point to a box, and those boxesÕ molecule lists must list the respective
