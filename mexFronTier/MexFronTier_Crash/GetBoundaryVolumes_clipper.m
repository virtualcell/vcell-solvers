...
scale = 2^52;
polyFront.x = int64(Front.Polygon(:,1)*scale);
polyFront.y = int64(Front.Polygon(:,2)*scale);
...
for i = 1:NumBoundaryPts
    ...
    polyVoronoiTemp.x = int64(V(R{i},1)*scale);
    polyVoronoiTemp.y = int64(V(R{i},2)*scale);
...
%     end
    if length(out)>1
        ...
        for iPolygon = 1:length(out)
            VertCoordsX1 = (out(iPolygon).x)'/scale;
            VertCoordsY1 = (out(iPolygon).y)'/scale;
        ...
        VertCoordsX1 = (out.x)'/scale;
        VertCoordsY1 = (out.y)'/scale;
        M = size(VertCoordsX1,2);
        VertCoordsX2 = [VertCoordsX1(2:M) VertCoordsX1(1)];
        VertCoordsY2 = [VertCoordsY1(2:M) VertCoordsY1(1)];
        EdgeVectorsX = VertCoordsX1-VertCoordsX2;
