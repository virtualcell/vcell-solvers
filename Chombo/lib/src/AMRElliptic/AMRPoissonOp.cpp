...
#ifdef FAS_HACKS
extern void DampBC( FArrayBox&,const Box&,const ProblemDomain&,Real,int);
#endif
// ---------------------------------------------------------
  ...
  if( !a_skip_res )
    {
      ...
      // damp
#ifdef FAS_HACKS
      DisjointBoxLayout dbl = a_scratch.disjointBoxLayout();
      DataIterator dit = a_scratch.dataIterator();
      for (dit.begin(); dit.ok(); ++dit)
	{
	  FArrayBox& res = a_scratch[dit];
	  DampBC( res, dbl[dit], m_domain, m_dx, m_refToCoarser );
	}
#endif
    }
  else 
...
////////////////////////////////////////////////////////////////////////
void convDiriBC_RBGS( FArrayBox&      a_state,
		      ...
		      )
{
  ...
  region.shift( a_dir, -sign(a_side) ); // a boarder

  FORT_GSRBLAPLACIAN(CHF_FRA(a_state),
		     CHF_CONST_FRA(a_rhs),
		     CHF_BOX(a_valid),
		     CHF_CONST_REAL(a_dx),
...
extern void convergeGS_BC( FArrayBox&,const FArrayBox&,const Box&,const ProblemDomain&,Real,int);
#endif
...
// hacks to do BCs right
#ifdef FAS_HACKS
	  convergeGS_BC(phiFab,a_rhs[dit],region,m_domain,m_dx,whichPass);
#endif
