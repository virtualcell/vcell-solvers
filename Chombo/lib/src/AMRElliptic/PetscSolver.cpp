...
// *******************************************************
template< >
void PetscSolverPoisson<LevelData<FArrayBox> >::define(LinearOp<LevelData<FArrayBox> >* a_operator,
                                                             bool a_homogeneous)
{
  PetscSolver<LevelData<FArrayBox> >::define(a_operator,a_homogeneous);
  if (m_alpha==0.0)
    { // put a small mass in to avoid singular systems
      CH_assert(this->m_dx!=0.0);
      m_alpha = m_beta/(this->m_dx*this->m_dx)*1.e-8;
    }
}
...
//
template< >
int PetscSolverPoisson<LevelData<FArrayBox> >::formMatrix( Mat a_mat, const LevelData<FArrayBox>& a_rhs,
							   PetscInt dummy_my0, PetscInt dummy_nloc,
							   PetscInt *dummy_d, PetscInt *dummy_o )
{
  CH_assert(a_rhs.nComp() == 1); CH_assert(this->m_dx!=0.0);
  DataIterator dit(a_rhs.disjointBoxLayout());
  int ierr, nc = 1;
  ...
  for ( dit = a_rhs.dataIterator() ; dit.ok() ; ++dit )
    {
      const Box &box = a_rhs.getBoxes()[dit];
      const FArrayBox &gidsFab = this->m_gids[dit()];
      BoxIterator bit(box);
      for (bit.begin(); bit.ok(); bit.next())
        {
          IntVect iv = bit();
          int ki = nc*(int)gidsFab(iv,0);
          int i = ki;
          // diag
                                                                    ...
                                                                    bool a_homogeneous )
{
  ...
  setVTParams(op->getAlpha(), op->getBeta(), &(*op->getACoef()), 
              &(*op->getEta()),
              &(*op->getLambda()));
}
...
//
template < >
int PetscSolverViscousTensor<LevelData<FArrayBox> >::formMatrix( Mat a_mat, const LevelData<FArrayBox>& a_rhs, 
								 PetscInt dummy_my0, PetscInt dummy_nloc,
								 PetscInt *dummy_d, PetscInt *dummy_o )
{
  DataIterator dit(a_rhs.disjointBoxLayout());
  CH_assert(a_rhs.nComp() == 2);
  CH_assert(m_dx>0.0);
  ...
  for ( dit = a_rhs.dataIterator() ; dit.ok() ; ++dit )
    {
      const Box &box = a_rhs.getBoxes()[dit];
      const FArrayBox &gidsFab = m_gids[dit()];
      ...
      for (bit.begin(); bit.ok(); bit.next())
        {
          ...
          IntVect iv = bit();
          int ki = nc*(int)gidsFab(iv,0),kiwst,kiest,kjsth,kjnth,kIwstJnth,kIestJnth,kIwstJsth,kIestJsth,jj;
          // get IV for stencil
          {
            ...
            kiwst  = nc*(int)gidsFab(ilft,0);
            kiest = nc*(int)gidsFab(iest,0);
            kjsth    = nc*(int)gidsFab(jsth,0);
            kjnth    = nc*(int)gidsFab(jnth,0);
            tempi[0] = -1; tempi[1] = 1; tempi += iv;
            kIwstJnth = nc*(int)gidsFab(tempi,0);
            tempi[0] = 1; tempi[1] = 1; tempi += iv;
            kIestJnth = nc*(int)gidsFab(tempi,0);
            tempi[0] = -1; tempi[1] = -1; tempi += iv;
            kIwstJsth = nc*(int)gidsFab(tempi,0);
            tempi[0] = 1; tempi[1] = -1; tempi += iv;
            kIestJsth = nc*(int)gidsFab(tempi,0);
          ...
          for (int kk=0,vv=1;kk<2;kk++,vv--)
            {
              const int ii = ki + kk; // the row (cell) we are working on
              Real vd,tt;
