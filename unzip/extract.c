...
static int extract_or_test_entrylist(__G__ unsigned numchunk,
                ulg *pfilnum, ulg *pnum_bad_pwd, LONGINT *pold_extra_bytes,
#ifdef SET_DIR_ATTRIB
                unsigned *pnum_dirs, dirtime **pdirlist,
#endif
...
static int TestExtraField(__G__ uch *ef, unsigned ef_len)
    __GDEF
    //uch *ef;
    //unsigned ef_len;
{
...
int memextract(__G__ uch *tgt, ulg tgtsize, ZCONST uch *src, ulg srcsize)  /* extract compressed */
...
int memflush(__G__ ZCONST uch *rawbuf, ulg size)
    __GDEF
    //ZCONST uch *rawbuf;
    //ulg size;
{
 ...
 */
uch *extract_izvms_block(__G__ ZCONST uch *ebdata, unsigned size, unsigned *retlen, ZCONST uch *init, unsigned needlen)
    __GDEF
    //ZCONST uch *ebdata;
    //unsigned size;
    //unsigned *retlen;
...
char *fnfilter(ZCONST char *raw, uch *space)         /* convert name to safely printable form */
    //ZCONST char *raw;
    //uch *space;
{
...
static int dircomp(ZCONST zvoid* a, ZCONST zvoid* b)   /* used by qsort(); swiped from Zip */
//    ZCONST zvoid *a, *b;
{
