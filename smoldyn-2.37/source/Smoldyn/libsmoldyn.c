...
extern "C" enum ErrorCode smolSetGraphicsParams(simptr sim,char *method,int timesteps,int delay) {
...
//	else if(i==(int)ECwildcard) {SmolClearError();i=-6;}		//??
...
//		if(i==-6) wcAddRule("molsetdifc",&molsetdifc,species,state,difc);		//??
		}
	if(drift) {
...
extern "C" int smolGetMoleculeCount(simptr sim,const char *species,enum MolecState state) {
	...
	return molcount(sim,i,NULL,state,NULL,-1);
 failure:
	return (int)Liberrorcode; }
...
extern "C" enum ErrorCode smolAddLatticePort(simptr sim, const char *lattice, const char *port) {
	...
	LCHECK(prt>=0,funcname,ECsame,NULL);
	simport=sim->portss->portlist[prt];
	er=latticeaddport(simlattice,simport);
	LCHECK(!er,funcname,ECmemory,"out of memory in latticeaddport");
	return ECok;
 failure:
	return Liberrorcode; }
...
extern "C" enum ErrorCode smolAddLatticeSpecies(simptr sim,const char *lattice, const char *species) {
	...
	i=smolGetSpeciesIndexNT(sim,species);
	LCHECK(i>0,funcname,ECsame,NULL);
	er=latticeaddspecies(simlattice,i);
