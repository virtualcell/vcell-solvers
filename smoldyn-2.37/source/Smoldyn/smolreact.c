...
// place products
	...
	for(prd=0;prd<nprod;prd++) {
		...
		if(rxn->rparamt==RPconfspread) {
			...
			mptr->serno=mptrallo->serno;
			for(d=0;d<dim;d++) {
				mptr->pos[d]=mptr->posx[d]=mptrallo->pos[d];
				mptr->posoffset[d]=mptrallo->posoffset[d]; }
			...
			if(prd==0) {
				dist=0;
				for(d=0;d<dim;d++) {
					v1[d]=mptr2->pos[d]-mptr1->pos[d];	// v1 is vector between centers
					dist+=v1[d]*v1[d]; }								// dist is squared separation between centers
				if(dist==0)
					if(rxn->rparam>=0) v1[0]=1;
					else v1[0]=2*sqrt(rxn->bindrad2);
				else {
					if(rxn->rparam>=0) {
						dist=sqrt(dist);
						for(d=0;d<dim;d++) v1[d]/=dist; }
					else {
						x=2*sqrt(rxn->bindrad2/dist)-1;
						for(d=0;d<dim;d++) v1[d]*=x; }}}

			mptrallo=(prd==0)?mptr1:mptr2;					// mptrallo is molecule that we're copying from
			mptr->mstate=rxn->prdstate[prd];
			mptr->box=mptrallo->box;
			mptr->serno=mptrallo->serno;
			for(d=0;d<dim;d++) {
				...
				mptr->posoffset[d]=mptrallo->posoffset[d];
				mptr->pos[d]=rxnpos[d]-v1[d]*rxn->prdpos[prd][0]; }
			mptr->pnl=(mptr->mstate==MSsoln)?NULL:mptrallo->pnl; }
		...
		if(rxn->logserno) {													// log reaction if needed
			...
			if(dorxnlog==1) {
				fptr=scmdgetfptr((cmdssptr) sim->cmds,rxn->logfile);
				if(!fptr) {
					simLog(sim,8,"cannot write to reaction log filename '%s'\n",rxn->logfile);
					dorxnlog=-1; }
				else {
					fprintf(fptr,"%g %s",sim->time,rxn->rname);
					for(d=0;d<dim;d++) fprintf(fptr," %g",rxnpos[d]);
					if(mptr1) fprintf(fptr," %li",mptr1->serno);
					if(mptr2) fprintf(fptr," %li",mptr2->serno);
					for(prd2=0;prd2<prd;prd2++) fprintf(fptr," ?");
					dorxnlog=2; }}
			if(dorxnlog==2) {
				fprintf(fptr," %li",mptr->serno);
				if(prd==nprod-1) fprintf(fptr,"\n"); }}}
