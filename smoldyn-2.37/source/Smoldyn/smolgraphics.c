...
int graphicsenablegraphics(simptr sim,char *type) {
	...
	if(code==-1) code=1;		// default graphics type
...
int graphicssettextitem(simptr sim,char *itemname) {
	...
	else if(sim->mols && ((i=readmolname(sim,itemname,&ms,0))>0 || i==-5 || i==-6) && ms!=MSbsoln);
...
void RenderMolecs(simptr sim) {
	...
	if(sim->latticess)
		for(lat=0;lat<sim->latticess->nlattice;lat++) {
			...
			copy_numbers=NULL;
			n=0;
			for(m=0;m<lattice->nspecies;m++) {		// m is the species identity
				ms=MSsoln;
				NSV_CALL(n = nsv_get_species_copy_numbers(lattice->nsv, lattice->species_index[m],&copy_numbers,&positions));
				for (i = 0; i < n; ++i) {						// n is the total number of molecules of this species, copy_numbers is how many in each site, and positions are the site positions
					if((mols->display[lattice->species_index[m]][ms]>0)&&(copy_numbers[i]>0)) {
						glBegin(GL_POLYGON);
						glColor3fv(gl2Double2GLfloat(mols->color[lattice->species_index[m]][ms],glf1,3));
						glVertex3f( positions[3*i+0]-lattice->dx[0]/2.0, positions[3*i+1]-lattice->dx[1]/2.0, positions[3*i+2]-lattice->dx[2]/2.0);       // P1
						glVertex3f( positions[3*i+0]-lattice->dx[0]/2.0, positions[3*i+1]+lattice->dx[1]/2.0, positions[3*i+2]-lattice->dx[2]/2.0);       // P2
						glVertex3f( positions[3*i+0]+lattice->dx[0]/2.0, positions[3*i+1]+lattice->dx[1]/2.0, positions[3*i+2]-lattice->dx[2]/2.0);       // P3
						glVertex3f( positions[3*i+0]+lattice->dx[0]/2.0, positions[3*i+1]-lattice->dx[1]/2.0, positions[3*i+2]-lattice->dx[2]/2.0);       // P4
						glEnd();
...
void RenderText(simptr sim) {
	...
	for(item=0;item<graphss->ntextitems;item++) {
		...
		else if(((er=molstring2index1(sim,itemname,&ms,&index))==0 || er>0 || er==-5) && ms!=MSbsoln)
			sprintf(string,"%s: %i",itemname,molcount(sim,er,index,ms,NULL,-1));
		else
