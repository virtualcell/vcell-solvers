...
int Geo_NearestLineSegPt(double *pt1,double *pt2,double *point,double*ans,int dim) {
	double x,thick;
	int d,edgenum;
...
int Geo_NearestTrianglePt2(double **point,double *norm,double *testpt,double *ans) {
	double *pt1,*pt2,*pt3,*en12,*en23,*en31,dot12,dot23,dot31,dx2,dy2,dz2,len2,dot;
	int d,corner,edgenum;
	...
	if(dot12<=0 && dot23<=0 && dot31<=0) {
		...
		if(dot12>-EPSILON) edgenum=1;
		else if(dot23>-EPSILON) edgenum=2;
		else if(dot31>-EPSILON) edgenum=3;
		else edgenum=0; }
...
int Geo_NearestCylinderPt(double *pt1,double *pt2,double rad,int dim,double *point,double *ans) {
	int d,edgenum;
	...
	dot/=len;																		// dot is not length of vect along cyl. axis, divided by the axis length
	for(d=0;d<dim;d++) vect[d]-=dot*(pt2[d]-pt1[d]);		// vect now points from cyl. axis to point
	if(dot<=EPSILON) {dot=0;edgenum=1;}
	else if(dot>=1-EPSILON) {dot=1;edgenum=2;}
	else edgenum=0;
	len2=0;
...
int Geo_NearestDiskPt(double *cent,double *axis,double rad,int dim,double *point,double *ans) {
	int d,edgenum;
	double dot,vect[3];
...
double Geo_LineExitCylinder(double *pt1,double *pt2,double *end1,double *end2,double rad,double *exitpt,int *exitend) {
	...
	Geo_NearestCylinderPt(end1,end2,rad,3,crsspt,exitpt);
