...
#
# $Id: TestXMLNode.py 12746 2011-02-03 15:58:47Z sarahkeating $
# $HeadURL: http://sbml.svn.sourceforge.net/svnroot/sbml/trunk/libsbml/src/bindings/python/test/xml/TestXMLNode.py $
#
...
class TestXMLNode(unittest.TestCase):
  ...
  def test_XMLNode_getChildForName(self):
    ...
    node = libsbml.XMLNode.convertStringToXMLNode(xmlstr,None)
    annotation = node.getChild("test")
    self.assert_( ( "test" != annotation.getName() ) == False )
    _dummyList = [ node ]; _dummyList[:] = []; del _dummyList
    pass  
  ...
  def test_XMLNode_getIndex(self):
    xmlstr = wrapString("<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>")
    node = libsbml.XMLNode()
    self.assert_( node.getIndex( "test") == -1 )
    _dummyList = [ node ]; _dummyList[:] = []; del _dummyList
    node = libsbml.XMLNode.convertStringToXMLNode(xmlstr,None)
    self.assert_( node.getIndex( "test") == 0 )
    _dummyList = [ node ]; _dummyList[:] = []; del _dummyList
    pass  
  ...
  def test_XMLNode_hasChild(self):
    xmlstr = wrapString("<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>")
    node = libsbml.XMLNode()
    self.assert_( node.hasChild( "test") == False )
    _dummyList = [ node ]; _dummyList[:] = []; del _dummyList
    node = libsbml.XMLNode.convertStringToXMLNode(xmlstr,None)
    self.assert_( node.hasChild( "test") == True )
    _dummyList = [ node ]; _dummyList[:] = []; del _dummyList
    pass  
