...
#
# $Id: TestXMLNode.rb 12746 2011-02-03 15:58:47Z sarahkeating $
# $HeadURL: http://sbml.svn.sourceforge.net/svnroot/sbml/trunk/libsbml/src/bindings/ruby/test/xml/TestXMLNode.rb $
#
...
class TestXMLNode < Test::Unit::TestCase
  ...
  def test_XMLNode_hasChild
    xmlstr = "<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>";
    node = LibSBML::XMLNode.new()
    assert( node.hasChild( "test") == false )
    node = nil
    node = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    assert( node.hasChild( "test") == true )
    node = nil
  end
