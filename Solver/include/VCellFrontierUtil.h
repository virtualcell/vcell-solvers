...
//	};
	...
	*/
	...
	inline void setRedistDefault(Front *front, int redist) {
		switch (redist) {
		case 1:
		case 2:
		  CURVE * curve;
		  ...
		  while (next_curve(front->interf,&curve)) {
			  curve->redist_order = redist;
		  }
		  break;
		default:
			VCELL_EXCEPTION(invalid_argument, "Invalid redist value " << redist << ", must be 1 or 2");
		}
	}
